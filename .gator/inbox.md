...
- **Phase 4 (Enterprise-side Gemini adapter)** — parent plan §6. Blocked only on Architect firing. Introduces Migration 011 (widened `vendor_session_id` uniqueness for Gemini duplicate-id-across-files) per §10 item 6 = (b) ratified path. Once it lands, the Phase 5 sweep (retire `extract-codex-sessions.py` + `extract-gemini-sessions.py` + `gator-session-common.py`) unblocks.
