...
**Phase 5 sweep (non-Enterprise session cleanup Phase 4) — COMPLETE 2026-08-16.** Atomic retirement per consumer-audit r3 §14.5: deleted `extract-codex-sessions.py` (477) + `extract-gemini-sessions.py` (410) + `gator-session-common.py` (372) — **~1,259 lines**; retired both archaeology TRIPWIREs ("Vendor Extractor Delegation Contract" + "get_machine_identity() duplicate copies") + the cross-cutting row_key TRIPWIRE; swept 6 charter files (session-archaeology rewritten to 2-file post-sweep shape, core-library, cross-cutting, fleet-intelligence, enterprise, INDEX). `gator_session_reader.get_machine_identity()` is sole owner. Base+contracts: **808 pass, zero regressions** (no test referenced the deleted files, as the consumer audit predicted). Base Gator now emits governance metadata only; Enterprise owns all vendor transcript custody — the tranche's guiding rule is now structurally true.

**Ready to pick up next**:
- **Phase 6 (Architect smoke vs widened vendor set)** — parent plan §8, the tranche's final phase. All three vendors have real custody on this machine (126+ sessions), so the protocol can run entirely locally.
- **Workflow B CDN-poll wrapper** — small release-hygiene follow-up (see open item below).
- **Next MINOR release** — Commits L + M + the sweep are all on `dev` unreleased; a v2.8.0 cut would ship the full Codex+Gemini audit surface.
