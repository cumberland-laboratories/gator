...
! On v2 repos, shipped content (scripts, procedures, reference-notes, blueprints, constitution, startup guide, charterignore) is routed to `.includes/`. Scaffolding files (`_template.md`, `_template.html`, `_template-narrative.html`, `README.md`) in shipped directories are routed to the user-visible root even on v2 — agents need them visible alongside user content. **v2.12.2 styling convention** (Architect-approved 2026-09-03): `_template-narrative.html` uses the snowpark-guide palette (`--teal` primary + `--red`/`--blue`/`--amber`/`--green` accents + gray-50→900 scale) with colored table-header variants (`table.table-blue`/`table-red`/`table-teal`), a five-variant callout vocabulary (default/warn/info/ok/note), a `.diagram` slot with colored `.diagram-node`+`.diagram-edge`, step-number color variants, and body layout `padding: 2rem 1.25rem` + `max-width: 1120px`. **v2.12.3 diagram layout fix**: `.diagram-edge` uses `flex: 1 1 auto` (not `flex: 0 0 auto`) so edges consume container slack — nodes distribute evenly across available width instead of clustering left with all whitespace piled on the right. Regression avoidance: any future `.diagram-*` CSS change must preserve the grow-behavior for edges so narrative HTML with a `.diagram` block reads correctly on wide viewports. `_template.html` and Gator's own `.gator/blueprints/charter-map.html` use v2.11.0's shipped Blueprints view aesthetic (`--bg: #fafbfc`, semantic node color names `--bp-core`/`governance`/`runtime`/`analysis`/`legacy`/`enterprise`, teal-governance header underline, 360px sidebar with `max-height: calc(100vh - 220px)`, translucent stage-hint pill). Changes to the shipped templates that alter the palette or class vocabulary must land the same class names in the reference implementation to keep fleet-repo output consistent with what Gator itself demonstrates. On v1 repos, everything goes to the flat root. The updater never flips the layout — it follows whichever layout it finds. v2.12.0 added the two HTML templates alongside `_template.md` under `blueprints/` for the `gator-blueprint-html-v1` protocol; see `scripts-layout.md` TRIPWIRE for the exact-set discipline. **v2.12.1 routing-consistency fix**: the scaffolding decision in `plan_updates()` (and its migrate_layout twin) must consult `gator_layout.USER_VISIBLE_SCAFFOLDING` directly — hardcoded local literal sets in the routing sites are silent drift and were the fleet-visible bug in v2.12.0 (both routing sites carried `{"README.md", "_template.md"}` locals that never grew when the HTML templates were added to the frozenset; result was fleet repos got HTML templates at `.includes/blueprints/`, invisible where agents look). Pin: `TestScaffoldingRoutingRoundTrip` in `tests/test_layout.py` — end-to-end pins on both routing sites + a source-grep pin that rejects the two-filename literal patterns. **Cumberland HTML reconciliation (Codex Sketch 2, 2026-09-12/13)**: post-Slice-3, `_template-narrative.html` carries the CSS core from the Cumberland master (`reference-notes/cumberland-html-document-template.html`) byte-for-byte inside `CUMBERLAND-NARRATIVE-STYLE:BEGIN/END` delimiters. `plan_updates()` routes both files unchanged (narrative Blueprint stays user-visible scaffolding at `.gator/blueprints/`; the Cumberland master is a shipped reference-note routed via `MIXED_DIRECTORY_SHIPPED_DEFAULTS["reference-notes"]`). The v2.12.2 palette note above is superseded by the Cumberland master's superset: extends the five-callout vocabulary to six (adds `.data` in purple), extends `.diagram-node` with `.purple`, adds `.pill.data`, adds a `.figure` block for hand-authored SVG, adopts justified body + hyphens + selective left-align overrides + h1 at 1.85rem with -0.01em tracking. Parity between master and narrative Blueprint pinned by `contracts/compatibility/test_cumberland_narrative_style_parity.py`; visual invariants (palette, callout/pill variants, typography, components) pinned by `contracts/compatibility/test_cumberland_visual_invariants.py`.
...
! Mixed directories (procedures, charters, blueprints, **reference-notes** since 2026-08-23) are split by template-derived classification: shipped files move, user files stay. Scaffolding (`_template.md`, `README.md`) is always preserved at root. Falls back to bootstrap defaults if template source unavailable. reference-notes moved out of Step 5's wholesale-move set when it was reclassified as a mixed directory (see `scripts-layout.md` — the fully-shipped treatment swept user notes into `.includes/` and bricked updates); Step 6's tuple must stay in sync with `MIXED_DIRECTORY_SHIPPED_DEFAULTS` keys. Pin: `test_shipped_dir_duplicates_get_removed` (rewritten with shipped filenames + a preserved user note).
! **`--dry-run` gates `--migrate-layout`** (2026-08-23): the gate fires in `main()` BEFORE template-source resolution and prints the current layout + how to run for real, executing nothing. Before the gate, `--migrate-layout --dry-run` ran the FULL migration — real file moves and hook regeneration under a flag that promises no changes (field case: a diagnostic dry-run migrated a user repo). There is no plan-only migrate; the gate refuses loudly instead of pretending. Pin: `TestMigrateDryRunGate::test_dry_run_does_not_migrate` (capfd, not capsys — `ensure_utf8_stdout()` re-wraps stdout).
...
### build_git_hook_wrappers() / _hook_shebang() / _format_shebang_refusal(result)
File: `src/gator_command/scripts/gator-update.py`
Generates the exact Python wrapper contents expected in the managed hook directory for `pre-commit`, `commit-msg`, and `post-commit`. Windows shebang is resolved via `gator_core.resolve_python_launcher_for_hooks()` (v2.10.0 Phase 2 cutover — the resolver is now the single seam for launcher truth, replacing the v2.9.3 inline probe). `_hook_shebang()` is a thin wrapper: on Windows it calls the resolver, returns `#!<resolver.path> -3` on resolved, raises `HookShebangUnresolvable(_format_shebang_refusal(result))` on degraded. `_format_shebang_refusal(result)` renders the resolver's structured `reason` + `checked` audit trail into an operator-actionable multi-line message. Unix uses `#!/usr/bin/env python3` (resolver returns `not-applicable`, wrapper short-circuits before consulting it). `subprocess.call` inside each wrapper uses exact `sys.executable` path. Each wrapper includes a guard that checks if `.gator/scripts/gator-pre-commit.py` exists before calling it. On branches where `.gator/` hasn't been merged yet, the guard prints a warning ("Proceeding in warning mode") and exits 0 — allowing the commit without blocking.
<- `plan_hook_updates()`, `install_git_hooks()`
-> `gator_core.resolve_python_launcher_for_hooks()` on Windows (import-inside-function so gator_core is resolved at call time — this is a hot path where the resolver's presence is expected but its exact identity depends on the running gator_core copy)
! The shebang and `subprocess.call` path solve different problems: the shebang must launch cleanly under Git (POSIX syntax cannot quote paths with spaces — the resolver enforces spaceless via `_validate_launcher_candidate`), while `subprocess.call` keeps the exact current interpreter even when its path contains spaces (safe there because it's a Python string, not a shebang).
! `HookShebangUnresolvable` bubbles up as a refusal — never a silent skip. `install_git_hooks` and `plan_hook_updates` catch it, print the actionable message to stderr, and return 0 / `[]`. This converts the pre-2026-08-28 silent-hook-install-succeeds-but-hook-broken failure mode into a discoverable install-time error. The v2.10.0 refactor changes only WHERE the resolution happens (gator_core.resolve_python_launcher_for_hooks), not the refusal contract.
! **Do not re-inline the shebang probe.** Every seam that needs launcher truth must go through `resolve_python_launcher_for_hooks()` so a user's `~/.gator/preferences.json` overrides one place, not many. The resolver's `source` field distinguishes user-configured from auto-detected — future callers may need that distinction.
...
! **`--dry-run --migrate-layout` is a refusal, not a plan.** The v2.9.2 gate (2026-08-23) refuses this combination — there is no plan-only migrate yet, so pretending to plan would misrepresent the operation. The `--json` axis on this refusal emits a structured payload (`{"schema": "gator-update-v1", "action": "migrate-layout-refused", "reason": ..., "layout": <v1|v2|mixed|null>}`) instead of prose (2026-08-28 fix — pre-fix the JSON branch further down was unreachable and machine consumers got human text). Exit code stays 0 for both channels; the payload's `action` field carries the "no work happened" signal.
...
! Regression pins: `tests/test_gator_hook.py` (35 — every mode × availability combination + TestResolveRepoRoot + TestResolveRepoRootAdditionalGuards + TestMainSeamWiring for the `main()` wiring itself) + `tests/test_hooks.py::TestPinAwareWrappers` (6).

! **Issue #32 fix (2026-09-16) — session-hook root resolution.** `session-open` and `session-start` may be invoked by an AI tool or vendor SessionStart entry point whose cwd is a governed repository SUBDIRECTORY (e.g. `repo/src/`). Before the fix, `main()` passed `Path.cwd()` directly to `resolve_governed_runtime`; the subdirectory was misclassified as ungoverned and the session hook skipped its existing behavior. Fix: new `_resolve_repo_root(cwd, hook_name)` helper + `_HOOKS_NEEDING_GIT_TOPLEVEL = {"session-open", "session-start"}` set. For hooks in that set, resolve the enclosing Git worktree's top level via `git rev-parse --show-toplevel`; for every other hook (commit-msg/pre-commit/post-commit/enforcer-review/approve), return cwd unchanged. Commit hooks are unchanged because git itself invokes them with cwd at the top level; enforcer-review and approve are user-driven verbs whose cwd expectations are owned by the caller. Fails open on any environment error (git missing from PATH, subprocess timeout, non-zero rc, empty output) — returns cwd so the existing ungoverned fallback handles the case, preserving the non-blocking guarantee for session hooks. Acceptance-coverage pins in `TestResolveRepoRoot` cover the six issue-#32 cases: repo root unchanged, governed subdir walks up, session-start pinned separately, linked worktree resolves to worktree top (not main repo), outside-git returns cwd, nested ungoverned Git inside governed parent stays at nested top (git rev-parse stops at first `.git` walking upward). `TestResolveRepoRootAdditionalGuards` pins the graceful degradation paths — git-missing-from-PATH returns cwd, enforcer-review + approve return cwd unchanged.

! **Runtime-split Phase 4 (2026-08-19) — repos no longer carry runtime.** Four coordinated changes, both gator-update copies + gatorize: (1) `plan_updates()` skips the `scripts` entry of `SHIPPED_TEMPLATE_DIRS` (constant kept for `migrate_layout` classification); (2) `main()` removes `.gator/.includes/scripts/` after a successful pin emission (guarded — removal failure never fails the update; v2-only, v1 repos migrate layout first); (3) `write_runtime_pin` callers pass `runtime_dir=templates_dir/"scripts"` so the manifest hashes the WHEEL runtime (the bytes actually in force under Variant A — also checkout-stable, sidestepping autocrlf for wheel-sourced manifests); (4) `build_git_hook_wrappers` stubs gained an ALWAYS-PRESENT fail-closed branch: pin present + repo script missing + dispatcher unreachable → pre-commit prints the pipx-install message and exits 1 (never commit ungoverned against a pin); commit-msg/post-commit exit 0 (never strand a mid-flight commit). `gatorize` fresh installs create no `.includes/scripts/` (contract: `test_gator_layout.py` asserts scripts-absence + valid wheel-sourced pin). `gator_layout.py::_has_required_includes_content` accepts scripts-absent-with-pin as valid v2 (S1 — see `scripts-layout.md`). HOOK_MAP gained `enforcer-review` (non-blocking, passthrough) as the post-removal enforcement invocation: `gator hook enforcer-review`. **Phase 4d (same day)**: HOOK_MAP also gained `approve` → `gator-approve.py` (non-blocking, passthrough for `--reason`/`--name`) — the Architect override path was a Phase-0 audit MISS (removed with the runtime, breaking the documented `python .gator/scripts/gator-approve.py` invocation). TRIPWIRE — the AGENT must never invoke `gator hook approve` (constitution: unauthorized self-approval); the dispatcher entry exists solely so the ARCHITECT has a machine-side path; reachability is pinned by test, not by execution. The 3 static bash hook files in `templates/scripts/hooks/` deleted (Phase-0 finding F2 — dead weight with v1 paths). Dogfooded on THIS repo: 33 repo-resident files removed; hook/session/init all verified live post-removal.

! **D6 staleness nudge + session-open pinned-repo regression fix (2026-08-22).** `gator-init.py` (both copies) prints the `policy_staleness_nudge` line in its banner (the agent-facing surface) — **r3 wiring fix**: the original call referenced an undefined `gator_dir` inside `print_boot_sequence()` and the blanket except swallowed the NameError, making the surface dead code (helper-only tests stayed green — the classic defensive-swallow + no-wiring-test hole). Correct name: `paths.gator_root`; wiring now pinned END-TO-END by `test_banner_shows_policy_nudge_wiring` (sentinel through the real banner path) + a helper-raises survival pin; `gator-session-open.py` emits it on STDERR (its contract forbids stdout). **Regression fix found while wiring**: session-open's sys.path bootstrap probed ONLY repo-resident script dirs and silently `return 0`'d when absent — on post-Phase-4 pinned repos the wheel copy was a NO-OP (hook self-heal lost; masked because `gator init` also heals and the exit-0 looked healthy). Own-dir fallback added (probe requires a `gator_core.py` sibling to qualify). Also: session-open's 20-hop and gator-approve's 10-hop walks uncapped (same class as the whiteboard r2 policies.py finding — arbitrary depth caps silently fail on deep working dirs).

! **Runtime-split Phase 3b (2026-08-19) — vendor SessionStart retarget.** The three vendor-hook JSON templates (`vendor-hooks/{claude-settings,codex-hooks,gemini-settings}.json`) now command `gator hook session-open` / `gator hook session-start` (CLI route via the dispatcher) instead of `python .gator/.includes/scripts/...`. **Whiteboard 2026-08-19 hardening (two rounds)**: the repo-scoped commands are FOUR-CLAUSE SHELL CHAINS — `gator hook <name> || python <repo-script> || python3 <repo-script> || py -3 <repo-script>` — CLI shim first (zero noise in the common case), then the three launcher realities: `python` on PATH, `python3`-only POSIX, `py`-launcher-only Windows. The second round (Codex Finding: `python`-only fallback contradicted the git-hook wrappers' own launcher-sensitivity rule at `_hook_shebang`/exact-interpreter embedding) added clauses 3-4; the terminal `py -3` clause live-verified by simulating all earlier clauses missing. `||` chains behave identically in cmd.exe and POSIX sh (127/"not recognized" → continue). Committed multi-machine strings can never embed machine paths — the chain is the maximum portable coverage, and every fallback clause dies with the repo scripts at Phase 4. Post-Phase-4 the fallback file disappears and the CLI clause is the only live path — the chain is honest transitional scaffolding. The merge logic's Gator-managed predicate `_is_gator_hook_command(cmd)` matches THREE shapes: `.gator/` substring (old-style AND the chain's fallback clause), bare `gator hook ` prefix, and the quoted absolute-launcher form Enterprise emits machine-side; missing any shape would duplicate the Gator group on migration. Migration contract pinned: old-style entries replaced in place, user hooks in the same group preserved (`tests/test_session_hooks.py::TestVendorHookMigration`, enterprise `TestGatorCommandPredicate` + `TestAbsolutizeCommands`).
