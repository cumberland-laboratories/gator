...
! **Error-taxonomy TRIPWIRE (2026-09-09 Codex F3)**: live-read handlers MUST catch `FileNotFoundError` and `PermissionError` before the general `OSError` branch and return **404** (not 500 with the exception text). A concurrently-removed or unreadable target would otherwise be distinguishable from the ordinary 404 path — a permission/existence oracle. Applies to both `_handle_file` (JSON envelope 404) and `_handle_raw` (raw HTML 404 via `_raw_error_from_req`). Order matters: `PermissionError` is an `OSError` subclass, so a bare `OSError` first would swallow the 404 taxonomy. Pinned by three tests: `test_read_exception_clause_names_both_error_classes` (source-grep asserting both exception classes appear in the specific clause BEFORE the general clause in both handlers), `test_file_unreadable_target_returns_404_on_posix` / `test_raw_unreadable_target_returns_404_on_posix` (POSIX `chmod 0` — makes containment resolve succeed and `read_bytes` raise `PermissionError`, deterministically reaching the remediated branch), plus containment-path pins `test_file_missing_target_returns_404_via_containment` / `test_raw_missing_target_returns_404_via_containment` (a missing target still returns 404, via the `_contained_repo_path` resolve rather than the read-time branch).
