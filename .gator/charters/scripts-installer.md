...
File: `src/gator_command/scripts/gatorize.py`
Ensures `.gitignore` contains standard rules for vault, `.vscode/`, `__pycache__/`, `.gator/active-vendor-session.json` (machine-local vendor session identity), `.gator/session-blocks/` (local-only transcript evidence), `.gator/diagnostics/` (bounded session-hook diagnostic log; see `log_hook_event()` below), hook ephemera (`.gator/whiteboard.md`, `.gator/commit_draft.md`, `.gator/status.json` — written and cleared each commit cycle, cause merge conflicts if tracked), and local agent companion files (`AGENTS.local.md`, `CLAUDE.local.md`, `GEMINI.local.md` — personal per-machine agent notes/skills; installer-untouched, never read or written by Gator, only gitignored). Appends missing rules. Also called by `gator-update.py` for gitignore convergence on existing repos — so the `.gator/diagnostics/` rule propagates fleet-wide on next update, keeping the log's private-diagnostic-residue contract intact without any per-repo setup step.
...
### main()
File: `src/gator_command/templates/gator-starter/scripts/gator-session-open.py`
Entry point for silent self-heal at vendor session start. Finds repo-level `.gator/`, resolves the scripts directory via a v2-first probe (`.gator/.includes/scripts/` → `.gator/scripts/` fallback), bootstraps sys.path from the resolved candidate, calls `get_gator_paths(repo_root)` from `gator_layout`, and passes the resulting `GatorPaths` (not a raw `Path`) into `ensure_git_hooks()`. Bails silently on `layout == "invalid"`. Captures `ensure_git_hooks()`'s return dict and routes non-happy-path statuses (per `gator_diagnostics.NON_HAPPY_STATUSES` = `{degraded, unavailable, error}`) into `gator_diagnostics.log_hook_event()` so the "silent self-heal" contract keeps stdout empty without keeping the maintainer blind. Always exits 0 — never blocks the vendor session. Never writes to stdout. Errors go to stderr with `gator-session-open:` prefix via the `__main__` guard's try/except.
@reads: `.gator/.includes/scripts/gator-init.py` (v2, primary) or `.gator/scripts/gator-init.py` (v1 fallback)
@writes: `.git/hooks/` (only when hooks are missing or stale), `.gator/diagnostics/hooks.log` (only on non-happy-path statuses; bounded to `MAX_LINES` entries; parent dir gitignored via `ensure_repo_gitignore()`)
...
! Prior v1-only implementation hardcoded `scripts_dir = str(gator_dir / "scripts")` and passed the raw `gator_dir` into `ensure_git_hooks()` which expects a `GatorPaths`. On v2 repos this raised `AttributeError` and the `__main__` guard's `except → sys.exit(0)` swallowed it, making the "silent self-heal" a silent no-op fleet-wide. The v2-first probe + `get_gator_paths()` is the fix; keep the `GatorPaths` contract with `ensure_git_hooks()` (see `gator-init.py::ensure_git_hooks()` for the reader side). Regression pin: `tests/test_session_open.py::TestMain::test_calls_ensure_git_hooks` — asserts the second arg to `ensure_git_hooks` is a `GatorPaths` (checks `.gator_root`, `.layout == "v2"`, `.scripts_dir`) so a regression to raw-Path signature fails on first attribute access.
! `gator_diagnostics` import is guarded — if the module is missing (old template on a not-yet-updated fleet repo), the script degrades to the pre-B3 behavior (still exit 0, still no stdout) and does not log. This keeps session-open forward-compatible across the fleet during the B3 template rollout.

### log_hook_event(gator_dir, script_name, status, detail="")
File: `src/gator_command/templates/gator-starter/scripts/gator_diagnostics.py`
Bounded, machine-local, append-only log helper for session-hook non-happy-path events. Writes one line per call to `.gator/diagnostics/hooks.log` in the form `<ISO-8601 UTC> <script_name> <STATUS> <detail> cwd=<cwd>`. Strips newlines from `detail` so a malformed detail cannot break the one-line-per-event contract. Enforces a `MAX_LINES = 200` cap by truncating oldest entries when the write would overflow. Every code path wrapped in try/except — the function MUST NOT raise, and MUST NOT write to stdout, because it's called from silent vendor SessionStart hooks.
@writes: `.gator/diagnostics/hooks.log` (best-effort)
<- `gator-session-open.py::main()` (currently the only caller)
! Log parent dir `.gator/diagnostics/` is gitignored by `ensure_repo_gitignore()` — the log is machine-local diagnostic residue, NOT repo knowledge. Never commit it.
! `NON_HAPPY_STATUSES = frozenset({"degraded", "unavailable", "error"})` is defined in the same module so callers don't hardcode the vocabulary. If `gator-init.ensure_git_hooks()` grows a new non-happy-path status, add it to this frozenset in the same commit so session-open picks it up automatically.
! Bounded-append implementation is O(N) per write when at/over the cap (reads all lines, keeps tail, rewrites). N is tiny (≤ 200) and rotation only fires when the cap is reached; the cost is trivial. Do NOT switch to time-based rotation without a corresponding change to test_session_hooks::TestDiagnosticsLog::test_rotation_keeps_last_max_lines.
