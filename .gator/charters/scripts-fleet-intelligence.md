...
-> [scripts-dashboard](scripts-dashboard.md) — dashboard consumes fleet-intelligence data; file list API now includes mtime; file scanner includes `.html`/`.htm` as of v2.4.5 (see scripts-dashboard "HTML file support"). **Blueprints 2.0 Release A (v2.11.0)**: the new `/api/repo/<name>/blueprint` endpoint does NOT consume fleet-intelligence data — it serves a static shipped payload gated to the Gator source repo. Later releases (charter-derived generation in Release B, drift map in Release E) will not depend on fleet-intelligence either, because the parser reads charter files directly rather than routing through fleet data collection. Blueprints is Dashboard-owned and charter-owned; fleet-intelligence has no role in the seam. (Post-Release-A whiteboard follow-up landed a `reason`-based split of the unavailable-state contract — see `scripts-cross-cutting.md § Repo-Scoped Dashboard Endpoints` and `scripts-dashboard.md::Blueprints view` for the invariant.)
-> [Index](INDEX.md)
