...
- `gator-repo-status.py` owns the per-repo deep status scan: charter coverage % (from `**Covers**:` declarations), stale charter detection, recent trailer history, override events, session summary count, and enforcement config (from `.gator/config.json`). Produces the `gator-repo-status-v1` JSON schema consumed by the Dashboard Repo view and Settings view. The Dashboard server (`gator-dashboard.py`) calls these scripts via subprocess — it owns HTTP serving, self-restart, git history endpoints, and file version serving (all git subprocess calls via `_git_run()` with UTF-8 encoding), not governance logic. In standalone dashboard mode, `gator-repo-status --path` is called per repo to build the fleet surface without `gator-fleet-report`. Dashboard restart adds `--no-open` to avoid duplicate browser tabs. **v2.13.0**: `gator-dashboard.py::main()` gained a `--port 0` kernel-assigned-port path and a column-0 `Ready on http://127.0.0.1:<port>/` protocol line for the dashboard-ui test harness; a `/api/__gator_debug/registry_state` endpoint gated on `GATOR_DASHBOARD_DEBUG=1` exposes the live `_REGISTRY_REPOS` global for read-side isolation pins. **v2.13.0 B1 Slice 1**: pure-Python content-transport helpers landed at module scope of `gator-dashboard.py` (`parse_logical_path`, `_parse_request`, `_is_reserved_windows_component`, response helpers `_send_json_error`/`_raw_error_direct`/`_dispatch_parse_error`, `_send_json` extended with `cache_control=`) and a new `dashboard/content_policy.py` module (allowlist/denylist policy constants, `_MIME_MAP`, `_serialize_listing_entry` wire-schema serializer). **v2.13.0 B1 Slice 2**: `do_GET` migrated in place — `_parse_request` runs EXACTLY ONCE per request from the top of `do_GET`; the four B1-owned endpoints (`/files`, `/file/`, `/raw/`, `/history/<file>`) dispatch through `_handle_files`/`_handle_file`/`_handle_raw`/`_handle_history` on `req.endpoint`. Four shipped route blocks (~350 lines) retired. Legacy branches (root shell, debug seam, `/api/data`, repo-scope `/history`, `/search`, `/commits`, Tier-2 status, static files, 404 fallback) preserved untouched. Discovery-serving symmetry restored via `_iter_scanner_files` (reparse-point-aware walker with junction/mount-point rejection before descent) + `_reparse_ls_tree_entry` (historical `git ls-tree` re-parse). Immutable-object-id `resolve_version_ref` + byte-preserving `git_show_at_ref` replace the pre-B1 ref-name-lookup + `_git_run.strip()` behavior. **Post-Codex remediation (2026-09-09)** extends the containment gate with a per-component reparse-point walk in `_contained_repo_path` (closing an in-repo policy alias), adds `no-store` on `/history?version=…` rejections, splits `FileNotFoundError`/`PermissionError` → 404 in live-read handlers, and splits the fixture so `beta` preserves `gator-command/` for the E1 positive live-listing round-trip. **Second-round remediation (same session, same day)** fixes a Python 3.9 crash in `_is_reparse_point` (the `Path.stat(follow_symlinks=)` kwarg is 3.10+; gate the DirEntry fast path with `isinstance(..., os.DirEntry)` and route Path inputs through `os.stat(os.fspath(x), follow_symlinks=False)`) and rewrites the F3 exception-branch tests to actually reach the remediated branch (POSIX `chmod 0` pattern + source-grep guard). **Third-round remediation (same session)** adds 6 cross-platform in-process F3 branch pins that inject a faulty target through the `_contained_repo_path` seam so the `except (FileNotFoundError, PermissionError)` clause fires deterministically on any platform, any Python version — closing the "privileged reader bypasses chmod 0" gap and finally making the `FileNotFoundError` branch executable in a test.

**v2.13.0 B2 Slice 1** (2026-09-10): sandboxed HTML preview server-side surface lands. `gator-dashboard.py::apply_html_csp_headers` is the additive-only CSP seam called from `_handle_raw` when the resolved MIME is `text/html*`; `apply_response_headers` (B1's transport-header helper) stays byte-exact preserved. `Sec-Fetch-Dest` selects between embedded CSP (iframe context) and external CSP (top-level navigation, with `sandbox allow-scripts;` prepended per r14 safer default); `Vary: Sec-Fetch-Dest` is unconditional. Fast-matrix audit `tests/test_shipped_template_audit.py` walks both shipped-template roots for CSP-`'unsafe-eval'`-requiring patterns (Function-constructor, eval, string-arg timers) so a future template's dynamic-code introduction fails CI before browsers catch it. Wasm compilation sinks are OUT OF SCOPE for the source-level audit — CSP-L3 §4.5.1 blocks them at runtime, so the browser is the backstop. **B2 Slice 2** (same day): browser-side surface — `views/repo.js` `loadFile()` HTML branch renders inline via `<iframe sandbox="allow-scripts">`, with Copy path / Refresh / Open-in-new-tab controls; three module-scope helpers factored (`buildRawUrl`, `copyPathFor`, `bindHtmlPreviewControls`). `dashboard.css` adds iframe base sizing (min-height 400px floor; Plan C amends the responsive flex chain). Fixtures: `_copy_shipped_blueprints(repo)` copies every `*.html` under both audit roots into the test repo pre-seed for the Slice 3 CSP-compat pin; `_csp_negative_control.html` deliberately violates three CSP directives for the Slice 3 detector-liveness pin. **B2 Slice 3** (same day, then three rounds of Codex remediation) locks the browser-side contract with **18 Playwright pins** in `tests/test_dashboard_ui/test_html_preview.py` — initial 13 (CSP variants, iframe sandbox exact-string, detector-liveness, shipped-template compat matrix over 9 files, parent-DOM isolation, inline-script execution, form-submission blocked, `apply_response_headers` byte-diff guard, URL canonical-prefix guard, Sec-Fetch-Dest fallback), then R1 (2026-09-10) adds 5 r14 §M1 production-path pins (Open-externally label + popup/CSP, Refresh + version preservation, Copy path, iframe 400px floor) plus retargets 4 existing pins at `_click_file_via_repo_view` (drives the shipped `views/repo.js::loadFile()` click path so the assertions cover the production trust boundary), plus rewires the CSP-violation listener as a postMessage-relay architecture (each document postMessage-relays its CSP violations up to `window.parent`; parent aggregates; compat matrix reads the parent aggregate to avoid `frame.evaluate`'s no-default-timeout hang on shipped templates that don't reach a stable execution context). R2 (2026-09-10) then tightens the liveness contract to assert BOTH lanes of the relay (child-side via `frame.evaluate` + parent-side via `page.evaluate`; single-lane assertion is a contract violation because the compat matrix reads only the parent aggregate), replaces the synthetic urllib CSP check in the popup pin with a captured browser Response (`page.context.on("response")`), and adds a rendered-pixel assertion (`getBoundingClientRect().height >= 400`) to the iframe sizing pin alongside the computed-style diagnostic. R3 (2026-09-10) extends `test_copy_path_writes_repo_relative_string` from a clipboard-only assertion to the full async flow — snapshot original icon → click → assert clipboard write + transient ✓ visible at 200ms (Promise microtask path) → wait 1600ms → assert original icon restored (setTimeout path) → `page.on("pageerror")` listener catches the `TypeError: Cannot set properties of null` a regression would emit; the pre-R3 pin only asserted the clipboard write and would have stayed green under the buggy `e.currentTarget` code because the Promise microtask often succeeds and only the delayed timer throws. R3 also reconciles the primary `scripts-dashboard.md` CSP-violation detector TRIPWIRE (now describes the two-lane liveness contract explicitly) and iframe sizing floor TRIPWIRE (now describes both rendered-pixel primary + computed-style diagnostic) to match the R2 implementations. Function-scoped `csp_violation_listener` fixture per r8 §M2. B2 arc converges. Data-collection surface unchanged — see [Dashboard charter](scripts-dashboard.md) `### main()`, `### do_GET(self)`, `### _send_dashboard_html(self)`, `### Dashboard UI test harness (Playwright, v2.13.0)`, `### content_policy.py`, `### _parse_request(handler)`, and [Contracts](contracts.md) `! B1 Dashboard content-transport response contract (v2.13.0)` for the full contract.
...
-> [scripts-dashboard](scripts-dashboard.md) — dashboard consumes fleet-intelligence data; file list API includes mtime; file scanner includes `.html`/`.htm` as of v2.4.5 (see scripts-dashboard "HTML file support"). **HTML blueprint artifacts (v2.12.0 protocol)** render through the existing `.html` file-browser surface — no dedicated endpoint, no fleet-intelligence role. v2.11.0's bespoke `/api/repo/<name>/blueprint` endpoint was retired in v2.12.0 in favor of the artifact-first architecture (see `scripts-dashboard.md::HTML artifact rendering` + `.gator/vault/artifacts/2026-09-02-html-artifact-protocol-implementation-plan.md`).
-> [Index](INDEX.md)
