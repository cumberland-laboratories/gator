...
! **Cumberland HTML document template (Codex Sketch 2 Slice 1, 2026-09-12; prose corrected 2026-09-13 F1)**: `cumberland-html-document-template.html` shipped at `templates/gator-starter/reference-notes/…` (source) with `.gator/.includes/reference-notes/…` as the dogfood mirror. **On v2 layouts this file belongs at `.gator/.includes/reference-notes/`; a fleet repo whose reference-notes/ carries this file at flat `.gator/reference-notes/` classifies as `mixed` (incomplete migration to `.includes/`).** Its filename is a member of `MIXED_DIRECTORY_SHIPPED_DEFAULTS["reference-notes"]` precisely to make `_has_legacy_shipped_content()` fire when the flat-root copy is present — removing membership would let the repo silently pass as clean v2 while the shipped default lives in the wrong location. This is distinct from the `USER_VISIBLE_SCAFFOLDING` scaffolding-only exemption, which applies only to `README.md`, `_template.md`, `_template.html`, `_template-narrative.html` — those DO stay user-visible at the flat root even on v2. Style vs role vs protocol are three separate concerns per the template's top-of-file comment: STYLE = Cumberland visual grammar (this file), ROLE = destination path (`artifacts/`, `blueprints/`, `procedures/`, `reference-notes/`, `threads/`), PROTOCOL = `gator-blueprint-html-v1` metadata (Blueprint-only). The `<style>` block encloses its full CSS between `/* CUMBERLAND-NARRATIVE-STYLE:BEGIN */` and `/* CUMBERLAND-NARRATIVE-STYLE:END */` delimiters — Slice 3 links this region byte-for-byte with `.gator/blueprints/_template-narrative.html` via `contracts/compatibility/test_cumberland_narrative_style_parity.py`. Pins: `TestResolveGatorLayout::test_mixed_defaults_includes_cumberland_html_template` (membership) + `TestResolveGatorLayout::test_v2_tree_with_cumberland_master_at_flat_root_is_mixed` (behavioral — construct the exact tree and assert `resolve_gator_layout == "mixed"`).
...
! **TRIPWIRE: `USER_VISIBLE_SCAFFOLDING` is a THREE-observer contract, not one.** The set is observed by (1) `_dir_is_scaffolding_only()` in this module (classifier), (2) `plan_updates()` in `gator-update.py` (routing on v2 update), and (3) `migrate_layout()` in `gator-update.py` (routing on v1→v2 migration). All three MUST consult the frozenset directly — hardcoded local literal sets in the routing sites are silent drift and were the exact fleet-visible bug in v2.12.0 (the two HTML scaffolding templates landed at `.includes/blueprints/` instead of the user-visible root because both routing sites carried `{"README.md", "_template.md"}` local sets that never got the HTML additions; fixed v2.12.1). Additions to the frozenset must be filename-exact AND paired with two regression pin classes: `TestResolveGatorLayout::test_v2_clean_when_blueprints_has_only_html_scaffolding` (classifier observer), plus `TestScaffoldingRoutingRoundTrip` (both routing observers — end-to-end pins that invoke `plan_updates` + `migrate_layout` and assert files land at `.gator/<dir>/`, not `.gator/.includes/<dir>/`, and a source-grep pin that rejects the two-filename literal patterns in `gator-update.py` source). v2.12.0 added `_template.html` and `_template-narrative.html`; the set is now four entries (`README.md`, `_template.md`, `_template.html`, `_template-narrative.html`).
