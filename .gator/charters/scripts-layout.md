...
! **Cumberland HTML document template (Codex Sketch 2 Slice 1, 2026-09-12)**: `cumberland-html-document-template.html` shipped at `templates/gator-starter/reference-notes/…` (source) with `.gator/.includes/reference-notes/…` as the dogfood mirror is the DEFAULT starting template for any HTML document Gator produces — the file every governed model reaches for when the Architect says "an HTML document that says X." Its filename is a member of `MIXED_DIRECTORY_SHIPPED_DEFAULTS["reference-notes"]` so a fleet repo whose reference-notes/ contains only this + scaffolding classifies as v2, not mixed. The `<style>` block encloses its full CSS between `/* CUMBERLAND-NARRATIVE-STYLE:BEGIN */` and `/* CUMBERLAND-NARRATIVE-STYLE:END */` delimiters — Slice 3 will link this region byte-for-byte with `.gator/blueprints/_template-narrative.html` via a compat test in `contracts/compatibility/`, so any local edit inside the delimiters must be replicated in both files or the compat check fails. Style vs role vs protocol are three separate concerns per the template's top-of-file comment: STYLE = Cumberland visual grammar (this file), ROLE = destination path (`artifacts/`, `blueprints/`, `procedures/`, `reference-notes/`, `threads/`), PROTOCOL = `gator-blueprint-html-v1` metadata (Blueprint-only). Pin: `TestResolveGatorLayout::test_mixed_defaults_includes_cumberland_html_template` (membership).
