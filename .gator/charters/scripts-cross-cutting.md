...
- `test_cumberland_narrative_style_parity.py` — byte-for-byte equality of the region between `CUMBERLAND-NARRATIVE-STYLE:BEGIN` and `CUMBERLAND-NARRATIVE-STYLE:END` in the master AND narrative Blueprint. Companion marker-existence pins so a "delimiters removed" regression fails with a specific message.
- `test_cumberland_visual_invariants.py` — palette tokens (13 hex-exact values + 6 light-alpha washes), callout variants (6 with matching label colors), pill variants (6), status badges (4), typography (h1 1.85rem + `-0.01em` tracking, body `text-align: justify + hyphens: auto`, subtitle same, selective left-align overrides for figure captions / footer / TOC), components (`.figure`, `.diagram-node.purple`, `.steps` + 4 counter variants, `pre code` reset, `.toc` blue-left-border), body scaffolding examples (one visible example of every callout variant + `.figure` + `.diagram` + `.steps` + a colored table in the master's body). 52 pins total. Fires on textual/regex assertions only — never on pixel snapshots (OS font-rendering variance rules those out).
- `tests/test_template_sync.py` — transitively extends parity to the shipped-template mirror pairs (source-repo master ↔ template mirror; source-repo narrative ↔ template mirror).
