...
- `test_cumberland_visual_invariants.py` — 57 pins: palette tokens, callout/pill/status-badge variants, typography (justified body + tracking + selective overrides), components (`.figure`, `.diagram-node.purple`, `.steps`, `pre code` reset, `.toc`, `.table-wrap`), body scaffolding examples, **full self-containment surface** (rebuilt via `html.parser` after 2026-09-13 F2 round-3): walks every start tag; covers `<script>`, `<img>` src+srcset, `<iframe>`, `<video>` src+poster, `<audio/source/track>`, `<embed>`, `<object>`, `<link>` non-nav, plus inline `style="…"` and `<style>` block CSS `@import`/`url(...)`. Handles unquoted attributes. Allows `data:` URIs, `#fragment` anchors, empty placeholders; `<a href>` navigation scoped-out. Meta-pin exercises every violation class (including the four F2 round-3 blind spots as explicit negative controls).
