...
- `test_cumberland_visual_invariants.py` — 71 pins: palette tokens, callout/pill/status-badge variants, typography (justified body + tracking + selective overrides), components (`.figure`, `.diagram-node.purple`, `.steps`, `pre code` reset, `.toc`, `.table-wrap`), body scaffolding examples, **full self-containment surface** (rebuilt via `html.parser` after 2026-09-13 F2 round-3; srcset parser + SVG image/use + quote-aware CSS url() added after 2026-09-13 round-4; SVG `<script>` + escape-aware unquoted CSS url() added after 2026-09-14 round-5; CSS hex escapes + `<iframe srcdoc>` recursion added after 2026-09-14 round-6; srcdoc double-decode fixed + depth cap fails closed after 2026-09-14 round-7; `<base href>` + labeled meta-refresh coverage after 2026-09-14 round-8; WHATWG meta-refresh parser + no-executable-scripts invariant after 2026-09-14 round-9; executable-surface scanner rebuilt via HTMLParser + srcdoc recursion + control-char scheme detection, and meta-refresh time-boundary tightened, after 2026-09-14 round-10; active-data-document guard for `iframe@src`/`frame@src`/`embed@src`/`object@data` × script-capable MIMEs after 2026-09-14 round-11): walks every start tag; covers `<script>`, `<img>` src+srcset, `<iframe>` src+srcdoc, `<video>` src+poster, `<audio/source/track>`, `<embed>`, `<object>`, `<link>` non-nav, SVG `<image>`/`<use>`/`<feImage>`/`<script>` (modern `href` + legacy `xlink:href`), `<base href>`, `<meta http-equiv=refresh>`, plus inline `style="…"` and `<style>` block CSS `@import`/`url(...)`. `srcset` parsed WHATWG-style so `data:` URIs with commas stay intact. CSS `url()` handles quoted URLs with internal whitespace, simple `\<char>` escapes, AND full CSS hex escapes `\<1-6 hex>[ws?]` (Chromium-verified). `<iframe srcdoc>` values are recursed through the same scanners at `depth+1` (cap 5, fails CLOSED beyond cap with `UNSCANNED-AT-DEPTH-N` sentinel) — no double-decode (HTMLParser already resolves attribute-value entities). Meta-refresh parsing runs through `_parse_meta_refresh_url` (round-9), a standalone WHATWG parser that accepts the `url=` label AS OPTIONAL — `0; https://example/`, `0, https://example/`, `0; url https://example/`, and `0.5; url=https://example/` all extract the target URL; `0` / `5` / `content=""` return None (in-place refresh, no violation). Round-10 F3 further requires a valid boundary (whitespace, `;`, `,`, or EOL) immediately after the time token — glued-time shapes like `0https://…` or `0foo` (which Chromium refuses) return None. **No-executable-scripts invariant** (round-9, rebuilt round-10): `test_no_executable_scripts_in_master` + `test_no_executable_scripts_in_narrative` forbid `<script>` blocks, inline `on*=` handlers, and `javascript:` URLs in both templates. Round-10 F1+F2 replaced the raw-regex helper with `_ExecutableScriptScanner` (HTMLParser-based, parallels `_HTMLResourceScanner`): parses real tags (comment-context `<script>` no longer false-positives), reads decoded attribute values (entity-obfuscated schemes like `java&#x73;cript:` still caught), normalizes ASCII control chars in URL schemes before checking (`_url_scheme_is_javascript`), and recurses `<iframe srcdoc>` at `depth+1` with the same `_MAX_SRCDOC_DEPTH=5` fail-closed discipline — a delayed inline script embedded in a srcdoc no longer slips past both the invariant AND the backstop grace window. `javascript:` flags only in the URL-carrying attribute contexts a browser actually resolves (`_JS_URL_ATTRS`) — prose mentions and `data-*` attributes are inert. Handles unquoted attributes. Allows `data:` URIs, `#fragment` anchors, empty placeholders; `<a href>` navigation scoped-out. Round-11 F1 adds an **active-data-document guard** wired into BOTH scanners: `_is_active_data_document` (backed by `_data_uri_active_mime`) flags a `data:` URI in an active-doc attribute context (`iframe@src` / `frame@src` / `embed@src` / `object@data`) when the MIME is script-capable (`text/html`, `application/xhtml+xml`, `image/svg+xml`, `application/xml`, `text/xml`) — the resource scanner surfaces it as an HTML resource violation, the executable scanner as a new `active-data-document` kind. Codex Chromium-verified that a `data:text/html,…<script>setTimeout(…,1200)…</script>` iframe executes past the browser backstop's grace window; the round-10 `_url_is_self_contained` accepted every `data:` URI and the executable scanner only recursed `srcdoc`, so the class bypassed both halves of the guarantee. Inert contexts (`<img src="data:image/png;…">`, `<a href="data:text/plain,…">`, CSS `url(data:font/…)`) and non-script MIMEs in active-doc contexts (`data:text/plain,…` inside an `<iframe>`) still pass. Thirteen meta-pins exercise every violation class as explicit negative controls, including double-encoded srcdoc literal (no false positive), three-level nested srcdoc external (flagged), beyond-cap srcdoc (fail-closed UNSCANNED sentinel), `_unescape_css` unit tests, round-8 indirect-fetches (`<base href>` + labeled meta-refresh), round-9 WHATWG meta-refresh parser unit-test (labeled + unlabeled + comma-sep + decimal-time + URL-less; round-10 extension adds glued-time negatives), round-9 no-executable-scripts helper unit-test, round-10 executable-surface helper unit-test covering entity/control-char schemes + srcdoc-nested delayed scripts + comment-context false-positive + prose-mention false-positive + beyond-cap fail-closed, and round-11 active-data-document meta-pin covering Codex\'s repro + all four browsing contexts × five script-capable MIMEs + inert-context negatives + non-script-MIME negatives + `_data_uri_active_mime` unit-checks.
- `test_cumberland_computed_style.py` — 12 Playwright pins. Narrow (375×667) parametrized over master + narrative Blueprint: page-level no-overflow; EVERY `<table>` has `.table-wrap` ancestor (universal); any oversized wrap has `overflow-x: auto` AND actually scrolls. Wide (1440×900) on master: h1 fontSize, body padding + max-width, header border color, section paragraph text-align. **Browser-level no-external-request backstop** (2026-09-14 round-8; tightened round-9): parametrized over both templates; routes every request via `page.route`, aborts anything not equal to the top-level template URL (sibling `file://` URLs are treated as external too — round-9 F2 fix), asserts zero non-template requests at load. Jointly definitive with the visual-invariants no-executable-scripts pins: those rule out JS-driven deferred fetches, this pin observes actual browser network. Round-8 wording called this pin definitive on its own — round-9 narrowed the claim to reflect that deferred JS-driven fetches beyond the short wait window would have slipped past without the no-script invariant in place.
