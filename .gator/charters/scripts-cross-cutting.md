...
- `test_cumberland_visual_invariants.py` — 57 textual/regex pins: palette tokens, callout/pill/status-badge variants, typography (justified body + tracking + selective overrides), components (`.figure`, `.diagram-node.purple`, `.steps`, `pre code` reset, `.toc`, `.table-wrap`), body scaffolding examples, **full self-containment surface** (post-2026-09-13 F3): scans `<script>`, `<img>`, `<iframe>`, `<video/audio/source/track>`, `<embed>`, `<object>`, `<link>`, plus inline CSS `@import` and `url(...)` — allows only `data:` URIs, `#fragment` anchors, and empty placeholders; `<a href>` navigation scoped-out. Meta-pin exercises the helper against synthesized violations.
...
- `test_cumberland_propagation.py` — 4 pins covering the shipped-delivery seam: master exists at canonical shipped-source path; **real wheel build** via session-scoped `python -m build --wheel` asserts the master appears at the exact package-data path inside the built wheel (closes the class of packaging-exclusion failure the earlier glob-literal pin would have missed); companion wheel pin asserts the full Cumberland-arc surface (master + narrative + constitution + authoring-procedure) all ship in the same wheel; `gator-update.plan_updates` routes the master to `.gator/.includes/reference-notes/…` on v2. Fresh-gatorize-installs-master is pinned in `test_gator_layout.py` (reuses the existing `action_install_gator` fixture).
