...
- `test_cumberland_visual_invariants.py` — visual invariants + bounded self-containment. **Round-12 closure (2026-09-14)**: replaced eleven rounds of blacklist scanners with a small positive-policy validator plus a pinned CSP `<meta>` in both templates. Module dropped from 2304 → 937 lines (−1367 net). Visual coverage unchanged (palette tokens / callouts / pills / status badges / typography / components / body scaffolding). Self-containment now runs `_validate_cumberland_document` — HTMLParser walk against `_ALLOWED_TAGS` (26 tags shipped in the templates: document skeleton, semantic sections, text, lists, tables, `<a>`, the SVG subset actually used) and per-tag `_ATTR_ALLOWLIST`. `<a href>` values constrained to fragment-only; `<meta http-equiv>` constrained to `content-security-policy` (`refresh` forbidden); every `on*=` event handler flagged; `<style>` DATA scanned for `@import` and `url(` (both forbidden); parse errors surface as `parse-error` findings (fail closed). Two shipped-template pins (`test_master_passes_positive_policy` + `test_narrative_passes_positive_policy`), positive-control minimal-document pin, and 30-parametrized rejection matrix (script / iframe / iframe-with-srcdoc / iframe-src-data-html / frame / object / embed / base / form / input / button / meta-refresh labeled + unlabeled / on-* / external-a-href / javascript-a-href / data-a-href / protocol-relative-a-href / css-@import / css-url / css-url-data / img / svg-script / svg-use-xlink / link / style-attr / video / audio) — rejecting the enclosing tag makes every prior parser-detail variant moot. Round-1..11 machinery (`_HTMLResourceScanner`, `_ExecutableScriptScanner`, `_parse_meta_refresh_url`, srcset parser, CSS hex-escape decoder, active-data-document guard, and their 12 meta-pins) DELETED per Codex's "material net reduction" stop criterion. **Pinned CSP** — both templates carry `<meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; script-src 'none'; img-src 'none'; font-src 'none'; frame-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'">` early in `<head>` (before `<title>` and `<style>`); presence + placement pinned. Defense in depth against future edits.
- `test_cumberland_computed_style.py` — visual layout + initial-load network smoke test. 12 Playwright pins. Narrow (375×667) parametrized over master + narrative Blueprint: page-level no-overflow; EVERY `<table>` has `.table-wrap` ancestor (universal); any oversized wrap has `overflow-x: auto` AND actually scrolls. Wide (1440×900) on master: h1 fontSize, body padding + max-width, header border color, section paragraph text-align. **Initial-load network smoke test** (2026-09-14 round-8, tightened round-9, scoped round-12): parametrized over both templates; routes every request via `page.route`, aborts anything not equal to the top-level template URL, asserts zero non-template requests during Chromium's initial load. Layer 3 of the bounded self-containment guarantee (Layer 1: positive policy; Layer 2: pinned CSP; Layer 3: this pin). NOT definitive on its own; the scoped claim is "no non-template network requests during Chromium's initial load".
