...
- `test_cumberland_visual_invariants.py` — 59 pins: palette tokens, callout/pill/status-badge variants, typography (justified body + tracking + selective overrides), components (`.figure`, `.diagram-node.purple`, `.steps`, `pre code` reset, `.toc`, `.table-wrap`), body scaffolding examples, **full self-containment surface** (rebuilt via `html.parser` after 2026-09-13 F2 round-3; srcset parser + SVG image/use + quote-aware CSS url() added after 2026-09-13 round-4; SVG `<script>` + escape-aware unquoted CSS url() added after 2026-09-14 round-5): walks every start tag; covers `<script>`, `<img>` src+srcset, `<iframe>`, `<video>` src+poster, `<audio/source/track>`, `<embed>`, `<object>`, `<link>` non-nav, SVG `<image>`/`<use>`/`<feImage>`/`<script>` (modern `href` + legacy `xlink:href`), plus inline `style="…"` and `<style>` block CSS `@import`/`url(...)`. `srcset` parsed WHATWG-style so `data:` URIs with commas stay intact. CSS `url()` handles quoted URLs with internal whitespace AND escape-aware unquoted URLs (`url(a\ b.png)` matches real browser behavior). Handles unquoted attributes. Allows `data:` URIs, `#fragment` anchors, empty placeholders; `<a href>` navigation scoped-out. Three meta-pins exercise every violation class as explicit negative controls.
