...
- `test_cumberland_visual_invariants.py` — 58 pins: palette tokens (13 hex-exact values + 6 light-alpha washes), callout variants (6 with matching label colors), pill variants (6), status badges (4), typography (h1 1.85rem + `-0.01em` tracking, body `text-align: justify + hyphens: auto`, subtitle same, selective left-align overrides for figure captions / footer / TOC), components (`.figure`, `.diagram-node.purple`, `.steps` + 4 counter variants, `pre code` reset, `.toc` blue-left-border, `.table-wrap` narrow-viewport safety net), body scaffolding examples (one visible example of every callout variant + `.figure` + `.diagram` + `.steps` + a colored table + `.table-wrap` wrap on the sample table), self-containment (no external stylesheet or external script src on either master or narrative). Textual/regex only — no pixel snapshots.
- `test_cumberland_computed_style.py` — 6 Playwright pins navigating the master via `file://` at 375×667 (narrow-viewport overflow safety net) and 1440×900 (computed anchors: h1 fontSize, body padding + max-width, header border color, section paragraph text-align). Computed-style values are browser-normalized numeric/string tokens — stable across platforms, NOT pixel snapshots.
- `test_cumberland_propagation.py` — 3 pins: master exists at the canonical shipped-source path; pyproject.toml `templates/**/*` package-data glob declared; `gator-update.plan_updates` routes the master to `.gator/.includes/reference-notes/…` on v2. Covers the wheel-contents / gatorize-installs / gator-update-plans acceptance surface Codex Sketch 2 asks for.
