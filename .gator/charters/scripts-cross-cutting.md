...
- `test_cumberland_visual_invariants.py` — 65 pins: palette tokens, callout/pill/status-badge variants, typography (justified body + tracking + selective overrides), components (`.figure`, `.diagram-node.purple`, `.steps`, `pre code` reset, `.toc`, `.table-wrap`), body scaffolding examples, **full self-containment surface** (rebuilt via `html.parser` after 2026-09-13 F2 round-3; srcset parser + SVG image/use + quote-aware CSS url() added after 2026-09-13 round-4; SVG `<script>` + escape-aware unquoted CSS url() added after 2026-09-14 round-5; CSS hex escapes + `<iframe srcdoc>` recursion added after 2026-09-14 round-6; srcdoc double-decode fixed + depth cap fails closed after 2026-09-14 round-7): walks every start tag; covers `<script>`, `<img>` src+srcset, `<iframe>` src+srcdoc, `<video>` src+poster, `<audio/source/track>`, `<embed>`, `<object>`, `<link>` non-nav, SVG `<image>`/`<use>`/`<feImage>`/`<script>` (modern `href` + legacy `xlink:href`), plus inline `style="…"` and `<style>` block CSS `@import`/`url(...)`. `srcset` parsed WHATWG-style so `data:` URIs with commas stay intact. CSS `url()` handles quoted URLs with internal whitespace, simple `\<char>` escapes, AND full CSS hex escapes `\<1-6 hex>[ws?]` (Chromium-verified). `<iframe srcdoc>` values are recursed through the same scanners at `depth+1` (cap 5, fails CLOSED beyond cap with `UNSCANNED-AT-DEPTH-N` sentinel) — no double-decode (HTMLParser already resolves attribute-value entities). Handles unquoted attributes. Allows `data:` URIs, `#fragment` anchors, empty placeholders; `<a href>` navigation scoped-out. Eight meta-pins exercise every violation class as explicit negative controls, including double-encoded srcdoc literal (no false positive), three-level nested srcdoc external (flagged), beyond-cap srcdoc (fail-closed UNSCANNED sentinel), and `_unescape_css` unit tests.
- `test_cumberland_computed_style.py` — 12 Playwright pins. Narrow (375×667) parametrized over master + narrative Blueprint: page-level no-overflow; EVERY `<table>` has `.table-wrap` ancestor (universal); any oversized wrap has `overflow-x: auto` AND actually scrolls. Wide (1440×900) on master: h1 fontSize, body padding + max-width, header border color, section paragraph text-align. **Browser-level no-external-request backstop** (2026-09-14 round-8): parametrized over both templates; routes every request via `page.route`, aborts anything not `file://`, asserts zero external URLs at load. Definitive self-containment check — catches any form the textual scanner misses.
