...
**v2.13.0 B2 Slice 3 (2026-09-10)** — Playwright pins that lock the browser-side surface. `tests/test_dashboard_ui/test_html_preview.py` lands 13 pins: `apply_response_headers` byte-diff guard (H1 seam invariant), non-Sec-Fetch-Dest → external CSP fallback (L2), Vary header on both variants (M1), embedded vs external CSP contents (M2/M3 — no `'unsafe-eval'`), non-HTML responses carry no CSP, URL canonical-prefix guard (r14 §M1 static check), iframe sandbox exact-string (r7 §H2), detector-liveness against the negative-control fixture (r7 §M5 mandatory), shipped-template compat matrix (parameterized over 9 files across both audit roots, 200 status precondition), parent-DOM isolation (r8 §M3), inline-script executes inside sandbox, form-submission blocked. Fixture `csp_violation_listener` is FUNCTION-scoped (r8 §M2 — pytest-playwright's `context` is function-scoped). `_load_raw_in_iframe` helper navigates parent to the dashboard origin FIRST (r14 §M1 addition — `frame-ancestors 'self'` blocks framing from opaque-origin `about:blank`/`data:` parents; the previous `set_content`-only approach silently produced `chrome-error://chromewebdata/` iframes that passed empty-violations assertions falsely). Playwright API details ratified through the review cycle: `element_handle().content_frame()` for a real `Frame`, `Locator.evaluate` for FrameLocator-scoped access, `expect(...).not_to_have_attribute(name, value)` for negation. Slice 3 concludes B2.
