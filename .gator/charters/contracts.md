...
| `contracts/compatibility/test_cumberland_visual_invariants.py` (Codex Sketch 2 Slice 4, 2026-09-13; extended by 2026-09-13 enforcer F2 + F4; self-containment surface broadened by 2026-09-13 F3 re-review) | 57 textual/regex pins asserting the load-bearing visual invariants of the Cumberland master. Palette (13 hex-exact tokens + 6 light-alpha washes, parametrized); callouts (default + 5 variants each with background + border-left + label color, plus `.callout.data` pinned to purple specifically); pills (6 variants); status badges (default + 3); typography (h1 `1.85rem` + `-0.01em` tracking; body `p` justified + `hyphens: auto`; subtitle same; selective left-align override for `.figure .caption` + `footer.doc-foot p` + `.toc p`); components (`.figure` block, `.diagram-node.purple`, `.steps` with 4 counter variants, `pre code` reset, `.toc`); delimiter contract (BEGIN/END markers must be inside the `<style>` block); body scaffolding examples (master body carries one visible example of every callout variant + `.figure` + `.diagram` + `.steps` + a colored table + `.table-wrap` on the sample table). **Full self-containment surface** (F3 re-review): shared `_find_external_html_resources` + `_find_external_css_resources` helpers scan for external references across `<script src>` + `<img src>` + `<iframe src>` + `<video/audio/source/track src>` + `<embed src>` + `<object data>` + `<link href>` non-nav + inline `@import` + inline `url(...)` — allow only `data:` URIs, `#fragment` anchors, and empty placeholders; `<a href>` navigation is scoped-out. Meta-pin `test_self_containment_helper_catches_known_forms` exercises the helper against synthesized violations so a regex bug is visible. Textual/regex only — no pixel snapshots. Computed-style checks live in the companion module below. | live `templates/…/cumberland-html-document-template.html`, `.gator/blueprints/_template-narrative.html` | nothing |
...
| `contracts/compatibility/test_cumberland_propagation.py` (Codex enforcer F4, 2026-09-13; tightened by 2026-09-13 F2 re-review) | 4 pins asserting the master travels the shipped delivery path to fleet repos: (1) Master exists at canonical shipped path under `templates/gator-starter/reference-notes/`. (2) **Real wheel build** via session-scoped `python -m build --wheel` fixture that runs once per test session and caches the resulting zip member list — asserts the master appears at the exact package-data path `gator_command/templates/gator-starter/reference-notes/cumberland-html-document-template.html` inside the built wheel. Closes the gap where the earlier glob-literal pin would have stayed green even if setuptools exclusions dropped the file at build time. (3) Companion wheel-content pin asserts the full Cumberland-arc delivery surface (master + narrative Blueprint + constitution + authoring-procedure) all appear inside the same built wheel. (4) `gator-update.plan_updates` — invoked in an isolated tmp_path v2 layout via `importlib.util` — produces a plan tuple `(add, source, dest)` whose dest lands at `.gator/.includes/reference-notes/cumberland-html-document-template.html`. Skips gracefully when `build` package is not importable or `gator-update.py` cannot be loaded. Fresh-gatorize-installs-master is pinned separately in `test_gator_layout.py` to avoid duplicating the install fixture. | live `templates/…/cumberland-html-document-template.html`, live source tree via `python -m build`, `gator-update.py`, `tmp_path` v2 seed | tmp `wheel_build/` (session-scoped) |
