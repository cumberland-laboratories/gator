...
| `contracts/compatibility/test_cumberland_visual_invariants.py` (Codex Sketch 2 Slice 4, 2026-09-13; extended by 2026-09-13 enforcer F2 + F4) | 58 pins asserting the load-bearing visual invariants of the Cumberland master template. Palette (13 hex-exact tokens + 6 light-alpha washes, parametrized); callouts (default + 5 variants each with background + border-left + label color, plus `.callout.data` pinned to purple specifically); pills (6 variants); status badges (default + 3); typography (h1 `1.85rem` + `-0.01em` tracking; body `p` justified + `hyphens: auto`; subtitle same; selective left-align override for `.figure .caption` + `footer.doc-foot p` + `.toc p`); components (`.figure` block, `.diagram-node.purple`, `.steps` with 4 counter variants, `pre code` reset, `.toc`); delimiter contract (BEGIN/END markers must be inside the `<style>` block); body scaffolding examples (master body carries one visible example of every callout variant + `.figure` + `.diagram` + `.steps` + a colored table); **F2 additions**: `.table-wrap` component defined + master body sample table wrapped in it; **F4 additions**: self-containment (no external stylesheet, no external script src) on both master and narrative. Textual/regex only — no pixel snapshots. Computed-style checks live in the companion module below. Skips gracefully when the master is absent (fleet repo pre-first-update). | live `templates/…/cumberland-html-document-template.html`, `.gator/blueprints/_template-narrative.html` | nothing |
| `contracts/compatibility/test_cumberland_computed_style.py` (Codex enforcer F2 + F4, 2026-09-13) | 6 Playwright pins that navigate to the master via `file://` and assert layout + computed-style at two viewports. **375×667 (narrow)**: `documentElement.scrollWidth <= innerWidth` (F2 overflow safety net); at least one `<table>` has a `.table-wrap` ancestor. **1440×900 (wide)**: h1 `getComputedStyle().fontSize == '29.6px'` (1.85rem @ 16px root); body padding `32px 20px` + max-width `1120px`; header border-bottom `3px solid rgb(25, 174, 184)` (teal #19aeb8); section > p `text-align == 'justify'`. Computed-style values are numeric/string normalized by the browser — stable across platforms — so these are NOT pixel snapshots (which Codex Sketch 2 rules out for OS font-rendering variance). `pytest.importorskip("playwright.sync_api")` at module scope; skips cleanly on installs without pytest-playwright. | live `templates/…/cumberland-html-document-template.html` via file:// URL | nothing |
| `contracts/compatibility/test_cumberland_propagation.py` (Codex enforcer F4, 2026-09-13) | 3 pins asserting the master travels the shipped delivery path to fleet repos. (1) Master exists at canonical shipped path under `templates/gator-starter/reference-notes/`. (2) pyproject.toml `[tool.setuptools.package-data]` declares `"templates/**/*"` glob (the packaging clause that puts every shipped template in the wheel). (3) `gator-update.plan_updates` — invoked in an isolated tmp_path v2 layout via `importlib.util` — produces a plan tuple `(add, source, dest)` whose dest lands at `.gator/.includes/reference-notes/cumberland-html-document-template.html`. Together the three pins cover Codex's F4 ask: wheel contains master, gatorize/update walk the template tree (via the glob), and gator-update routes the master to the v2-correct destination. Skips cleanly when `gator-update.py` cannot be loaded. | live `pyproject.toml`, live `templates/…/cumberland-html-document-template.html`, `gator-update.py`, `tmp_path` v2 seed | nothing |
