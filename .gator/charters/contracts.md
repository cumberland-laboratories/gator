...
| `contracts/compatibility/test_cumberland_visual_invariants.py` (Codex Sketch 2 Slice 4, 2026-09-13; extended by 2026-09-13 enforcer F2 + F4; self-containment surface broadened by 2026-09-13 F3 re-review; rewritten via `html.parser` after 2026-09-13 F2 round-3 re-review; srcset parser + SVG image/use + quote-aware CSS url() added after 2026-09-13 round-4 re-review; SVG `<script>` + escape-aware unquoted CSS url() added after 2026-09-14 round-5 re-review; CSS hex escapes + `<iframe srcdoc>` recursion added after 2026-09-14 round-6 re-review; srcdoc double-decode fixed + depth cap fails closed after 2026-09-14 round-7 re-review; `<base href>` + meta-refresh coverage after 2026-09-14 round-8 re-review; WHATWG meta-refresh parser + no-executable-scripts invariant after 2026-09-14 round-9 re-review; executable-surface scanner rebuilt via HTMLParser + srcdoc recursion + control-char scheme detection, and meta-refresh time-boundary tightened, after 2026-09-14 round-10 re-review; active-data-document guard for `iframe@src`/`frame@src`/`embed@src`/`object@data` × script-capable MIMEs after 2026-09-14 round-11 re-review) | 71 pins asserting the load-bearing visual invariants of the Cumberland master. Palette (13 hex-exact tokens + 6 light-alpha washes, parametrized); callouts (default + 5 variants each with background + border-left + label color, plus `.callout.data` pinned to purple specifically); pills (6 variants); status badges (default + 3); typography (h1 `1.85rem` + `-0.01em` tracking; body `p` justified + `hyphens: auto`; subtitle same; selective left-align override for `.figure .caption` + `footer.doc-foot p` + `.toc p`); components (`.figure` block, `.diagram-node.purple`, `.steps` with 4 counter variants, `pre code` reset, `.toc`); delimiter contract (BEGIN/END markers must be inside the `<style>` block); body scaffolding examples (master body carries one visible example of every callout variant + `.figure` + `.diagram` + `.steps` + a colored table + `.table-wrap` on the sample table). **Full self-containment surface** — the F3 regex-only scanner was rebuilt via stdlib `html.parser.HTMLParser` to close the F2 round-3 blind spots (unquoted attribute values, `srcset` comma-lists, `<video poster>`, inline `style="…"` carrying `url(...)`/`@import`). `_HTMLResourceScanner` walks every start tag; `_scan_css_for_externals` extracts `@import` + `url()` from any CSS chunk (used by both `<style>` blocks AND inline style attributes). Coverage: `<script src>`, `<img src/srcset>`, `<iframe src>`, `<video src/poster>`, `<audio src>`, `<source src/srcset>`, `<track src>`, `<embed src>`, `<object data>`, `<input src>`, `<link href>` non-nav (with `<a href>` navigation scoped-out), SVG `<image>`/`<use>`/`<feImage>`/`<script>` (modern `href` + legacy `xlink:href`), plus inline CSS. `srcset` uses a WHATWG-style whitespace-boundary parser so commas inside `data:` URIs stay part of the URL (naive comma-split reported base64 tail fragments as external URLs — F1 round-4). CSS `url(...)` handles three quoting shapes (double-quoted, single-quoted, unquoted) with an ESCAPE-AWARE unquoted char class covering BOTH the simple `\<char>` form (F2 round-5) AND full CSS hex escapes `\<1-6 hex digits>[optional whitespace terminator]` (F1 round-6). `_unescape_css` decodes hex escapes to `chr(int(hex, 16))` then unwraps simple escapes, so `url(\68 ttps://...)` normalizes to `url(https://...)` before the self-containment check — Chromium-verified. `<iframe srcdoc>` values are RECURSED through the same scanners at `depth+1` (capped at `_MAX_SRCDOC_DEPTH=5`) — external resources embedded inside a srcdoc document (img, script, `<style>` block, inline style) are surfaced with an `iframe@srcdoc>` scope prefix in the violation tag (F2 round-6). No `html.unescape` is applied on the srcdoc value — `html.parser.HTMLParser` already decodes character references in attribute values, so a second unescape would false-positive on legitimate literal `&lt;`/`&gt;` inside srcdoc (F1 round-7 fix). Beyond the depth cap the scanner FAILS CLOSED — emits an `iframe@srcdoc>UNSCANNED-AT-DEPTH-N` violation instead of silently skipping (F2 round-7 fix; silent-skip contradicted the self-containment guarantee because Chromium fetches at any depth). Meta-refresh parsing follows WHATWG "shared declarative refresh steps" via `_parse_meta_refresh_url` (F1 round-9): the `url=` label is OPTIONAL — `0; https://example/` and `0, https://example/` both flag as external. The round-8 regex hard-required `url=` and reported unlabeled forms clean; round-9 replaces it with a standalone WHATWG parser that also handles comma separator, decimal time, keyword-without-`=`, and quoted-URL shapes. In-place refresh directives (`content="5"`, `content="0"` — no URL) return None from the parser and never synthesize a meta-refresh violation. Round-10 F3 adds a boundary requirement immediately after the time token — whitespace, `;`, `,`, or EOL — so glued-time content like `0https://example/` or `0foo` (which Chromium refuses) no longer parses as an external navigation target. **No-executable-scripts invariant** (F2 round-9, rebuilt round-10): `test_no_executable_scripts_in_master` + `test_no_executable_scripts_in_narrative` scan both templates via `_find_executable_script_surface` and forbid `<script>` blocks, inline `on*=` event handlers, and `javascript:` URLs. Round-10 F1+F2 replaced the round-9 raw-regex helper with `_ExecutableScriptScanner`, an HTMLParser-based walk parallel to `_HTMLResourceScanner`: it (a) parses real tags so `<script>` inside HTML comments no longer false-positives, (b) reads DECODED attribute values so entity-obfuscated `javascript:` schemes (`java&#x73;cript:`, `java&#x09;script:`) are still caught, (c) checks URL scheme AFTER browser-style control-char stripping via `_url_scheme_is_javascript` (ASCII TAB/LF/CR/FF/NUL are stripped by Chromium during scheme detection), and (d) recurses `<iframe srcdoc>` at `depth+1` with the same `_MAX_SRCDOC_DEPTH=5` fail-closed cap the resource scanner uses — a delayed inline script embedded in a srcdoc no longer slips past both the invariant and the backstop's 200 ms grace window. `javascript:` schemes are only flagged in the URL-carrying attribute contexts a browser actually resolves (`_JS_URL_ATTRS`); prose containing the literal string "javascript:" or `data-*` attributes holding it do not false-positive. This invariant is the partner of the browser backstop in `test_cumberland_computed_style::test_template_makes_no_external_requests_at_load` — with no executable surface anywhere in either template's tag tree (including srcdoc-nested content), no code can schedule a deferred fetch, so the two pins are jointly definitive for the self-containment guarantee. **Active-data-document guard** (round-11 F1): `_is_active_data_document` (backed by `_data_uri_active_mime`) treats a `data:` URI in a browsing/plugin attribute context (`iframe@src`, `frame@src`, `embed@src`, `object@data`) with a script-capable MIME (`text/html`, `application/xhtml+xml`, `image/svg+xml`, `application/xml`, `text/xml`) as a violation in BOTH scanners — resource scanner surfaces it as an HTML resource violation, executable scanner surfaces it under the new `active-data-document` kind. Codex Chromium-verified that `<iframe src="data:text/html,…<script>setTimeout(…,1200)…</script>">` executes past the 200 ms browser-backstop grace window; the round-10 `_url_is_self_contained` accepted every `data:` URI unconditionally and the round-10 executable scanner only recursed `srcdoc`, so the class survived both halves. `data:` in INERT contexts (`<img src="data:image/png;…">`, `<a href="data:text/plain,…">`, CSS `url(data:font/woff,…)`) still passes — the browser paints or downloads bytes, never executes — so Cumberland's legitimate inline-image use pattern is preserved. Non-script MIMEs (`data:text/plain,…`, `data:image/png;…`, `data:,body-only`) in active-doc contexts also pass. Thirteen meta-pins exercise every violation class as explicit negative controls (F3 round-2 baseline + F2 round-4 SVG/srcset/quoted-url + F1/F2 round-5 SVG-script/escaped-space + F1/F2 round-6 hex-escape/srcdoc + F1/F2 round-7 srcdoc-double-decode-negative + within-cap + beyond-cap + `_unescape_css` unit-test meta-pin covering greedy 6-digit hex behavior + F1 round-8 indirect-fetches for `<base href>`/meta-refresh + F1 round-9 WHATWG meta-refresh parser unit-test (extended round-10 with glued-time negative controls) + F2 round-9 no-executable-scripts helper meta-pin + F1/F2 round-10 helper meta-pin covering entity/control-char/srcdoc-nested/comment-context/prose-mention/beyond-cap + F1 round-11 active-data-document meta-pin covering Codex repro + all four browsing contexts × five script-capable MIMEs + inert-context negatives + non-script-MIME negatives + `_data_uri_active_mime` unit-checks). Allows `data:` URIs, `#fragment` anchors, empty placeholders. Meta-pin `test_self_containment_helper_catches_known_forms` exercises every violation class (including the F2 round-3 blind spots as explicit negative controls) so a parse/predicate bug is visible immediately. Textual only — no pixel snapshots. Computed-style checks live in the companion module below. | live `templates/…/cumberland-html-document-template.html`, `.gator/blueprints/_template-narrative.html` | nothing |
| `contracts/compatibility/test_cumberland_computed_style.py` (Codex enforcer F2 + F4, 2026-09-13; F4 re-review parametrized + universalized 2026-09-13; browser-level backstop added 2026-09-14 round-8; route-filter tightened + wording narrowed 2026-09-14 round-9) | 12 Playwright pins navigating both Cumberland anchor files via `file://` and asserting layout + computed-style. **Narrow (375×667) — parametrized over master + narrative**: `documentElement.scrollWidth <= innerWidth` (F2 overflow safety net); EVERY `<table>` has a `.table-wrap` ancestor (universal, not existential); any oversized `.table-wrap` has `overflow-x: auto` computed AND actually scrolls. **Wide (1440×900) — master only**: h1 `fontSize == '29.6px'`; body padding `32px 20px` + max-width `1120px`; header border-bottom `3px solid rgb(25, 174, 184)` (teal); section > p `text-align == 'justify'`. **Browser-level no-external-request backstop** (round-8 + round-9): parametrized over both templates; `page.route("**/*", …)` intercepts every request and, per round-9's F2 tightening, aborts anything not equal to the exact top-level template URL — sibling `file://` URLs are treated as external too, so an iframe pulling in a Cumberland neighbour no longer looks self-contained. Zero non-template requests must be recorded at load. Jointly with the sibling `test_no_executable_scripts_in_{master,narrative}` invariants (visual-invariants module, round-9), this pin is definitive: the invariants rule out any JS surface capable of scheduling a deferred fetch, and this pin observes the actual browser network at load. The round-8 wording called this pin definitive on its own; that overstated the guarantee because deferred JS-driven fetches beyond the 200 ms grace window would have slipped past — the round-9 no-script invariant closes that hole. Computed-style values are browser-normalized numeric/string tokens — NOT pixel snapshots. | live `templates/…/cumberland-html-document-template.html`, `.gator/blueprints/_template-narrative.html` via file:// | nothing |
