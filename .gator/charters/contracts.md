...
| `contracts/compatibility/test_cumberland_visual_invariants.py` (Codex Sketch 2 Slice 4, 2026-09-13) | 52 pins asserting the load-bearing visual invariants of the Cumberland master template. Palette (13 hex-exact tokens + 6 light-alpha washes, parametrized); callouts (default + 5 variants each with background + border-left + label color, plus `.callout.data` pinned to purple specifically); pills (6 variants); status badges (default + 3); typography (h1 `1.85rem` + `-0.01em` tracking; body `p` justified + `hyphens: auto`; subtitle same; selective left-align override for `.figure .caption` + `footer.doc-foot p` + `.toc p`); components (`.figure` block, `.diagram-node.purple`, `.steps` with 4 counter variants, `pre code` reset, `.toc`); delimiter contract (BEGIN/END markers must be inside the `<style>` block); body scaffolding examples (master body carries one visible example of every callout variant + `.figure` + `.diagram` + `.steps` + a colored table). Textual/regex only — no Playwright, no computed-style snapshotting, no pixel-based checks (Codex Sketch 2 explicitly ruled those out for OS font-rendering variance). Since Slice 3 pins the master and narrative Blueprint byte-identical, invariants tested against the master transitively apply to the narrative Blueprint. Skips gracefully when the master is absent (fleet repo pre-first-update). | live `templates/…/cumberland-html-document-template.html` | nothing |
| `contracts/compatibility/test_cumberland_narrative_style_parity.py` (Codex Sketch 2 Slice 3, 2026-09-12) | `test_master_contains_begin_and_end_markers`, `test_narrative_contains_begin_and_end_markers`, `test_cumberland_shared_region_is_byte_identical` — extracts the region between `CUMBERLAND-NARRATIVE-STYLE:BEGIN` and `CUMBERLAND-NARRATIVE-STYLE:END` markers from the Cumberland master (`templates/gator-starter/reference-notes/cumberland-html-document-template.html`) AND the narrative-Blueprint specialization (`.gator/blueprints/_template-narrative.html`), asserts byte-for-byte equality. Compact-diff failure message points at the first divergent line. Slice 3 reconciled the two files by replacing the narrative Blueprint's `<style>` body with the master's exact bytes; this test locks the invariant. Marker-existence pins are separate tests so a "markers removed" regression fails with a clearer message than a bytes-diff would produce. Slice-3-specific because the two mirror pairs (source-repo master + template mirror; source-repo narrative + template mirror) are pinned identical by `tests/test_template_sync.py` — extending parity to the shipped copies transitively. | live `templates/…/cumberland-html-document-template.html`, live `.gator/blueprints/_template-narrative.html` | nothing |
