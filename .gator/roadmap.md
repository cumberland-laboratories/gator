...
| # | Feature | Status | Notes |
|---|---------|--------|-------|
| 1 | **`promote-to-pypi.yml` CDN-lag fix** | Considering | Recurring paper cut — v2.5.2, v2.5.3, v2.5.4, v2.6.0 all failed the post-publish smoke because PyPI's CDN needs ~30-60s before pip sees the new version. Publish itself always succeeds. Fix: `sleep 60` or poll-until-visible before smoke. Small, cheap; ship in v2.6.1. |
| 2 | **`commits.py` docstring + fallback arg-order fix** | Considering | The CLI code itself works (`commits transcripts <sha>`), but the module docstring and `handle()` fallback usage message say `commits <sha> transcripts` — a real evaluator-mislead vector caught during smoke-test Run 1 (Finding #8). Cosmetic; safe alongside item 1 in v2.6.1. |
