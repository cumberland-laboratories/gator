...
Updated 2026-08-13 (v2.6.0 shipped to PyPI + GitHub Release 2026-08-11; **v2.6.1 release-prep cut 2026-08-13 on `dev`** — session cleanup Phase 2 + Phase 3 + two paper-cut fixes; awaiting RC tag → PyPI promote). Priority-ordered within each track.
...
Ordered roughly by cost within groupings. Items 1-2 are quick wins worth folding into a v2.6.1 release. Items 3-8 are the arc toward a defensible public Enterprise announcement (single-pipx install, cross-OS defaults, cleanup of inert Enterprise-side session-block code). Items 9-11 close out the non-Enterprise session cleanup started 2026-08-12: two Enterprise-side vendor adapters (Codex, Gemini) that must ship before Phase 4 sweeps `gator-session-common.py` + the two remaining base extractors. Item 12 is a follow-on surfaced during that cleanup.

| # | Feature | Status | Notes |
|---|---------|--------|-------|
| 1 | **`promote-to-pypi.yml` CDN-lag fix** | Landed (dev) | Recurring paper cut — v2.5.2, v2.5.3, v2.5.4, v2.6.0 all failed the post-publish smoke because PyPI's CDN needs ~30-60s before pip sees the new version. Publish itself always succeeds. **Landed 2026-08-13** as a bounded poll-until-visible step (8×15s = 120s max) sequenced before the install step in `post-verify`. Real validation on next release cycle. |
| 2 | **`commits.py` docstring + fallback arg-order fix** | Landed (dev) | The CLI code itself works (`commits transcripts <sha>`), but the module docstring and `handle()` fallback usage message said `commits <sha> transcripts` — a real evaluator-mislead vector caught during smoke-test Run 1 (Finding #8). **Landed 2026-08-13**: docstring rewritten (with the verb-first-vs-URL-shape rationale spelled out), fallback usage message corrected, `main.py` register-comment fixed, roadmap "What Enterprise adds" example command corrected. CHANGELOG.md v2.6.0 entry left as-is (historical); v2.6.1 CHANGELOG addition at release-prep will note the fix. |
...
| 9 | **Enterprise-side Codex adapter** | Considering | Add Codex CLI transcript discovery + ingest to Enterprise-cli's `transcripts_discovery.py` (currently Claude-only, `~/.claude/projects/`). Codex vendor coverage regression risk — deferred from non-Enterprise session cleanup Phase 3 per §5 decision 2(b) so base Gator retains its `extract-codex-sessions.py` archaeology path until the Enterprise equivalent ships. **Blocks Phase 4 of the session cleanup** (which retires `extract-codex-sessions.py`, `extract-gemini-sessions.py`, and `gator-session-common.py` atomically). |
