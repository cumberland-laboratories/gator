...
def render_entry_content(has_command_post, agent_type="claude"):
    ...
    gator_init_block = (
        ...
        "(4) follow the 10 rules in the protocol exactly. "
        "If it is not your turn (exit code 1), escalate first if you have a genuine "
        "Architect-owned blocker; otherwise run "
        "`gator loop wait --token <your-token>` to block until the loop becomes actionable. "
        "Submit artifacts only through "
        "the CLI (`gator loop submit-draft` or `gator loop submit-review`). "
        "Never edit loop directory files directly. "
        "Every plan and findings submission must include a `## Executive Summary` section "
        "(four bullets or ~120 words) — the Dashboard extracts it for at-a-glance inspection.\n\n"
