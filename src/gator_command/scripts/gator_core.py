...
def is_enterprise_active(gator_dir):
    """Return True iff Enterprise capability is configured on this repo.

    Canonical presence-detection check for the .gator/enterprise.json marker.
    Reads the marker per the schema at contracts/schemas/enterprise-config.json.
    Fail-closed: unreadable marker, malformed JSON, or `enabled != True` all
    return False. Matches the reference implementation in
    contracts/compatibility/test_enterprise_marker.py::_is_enterprise_active
    and is expected to stay byte-behaviorally identical to it —
    tests/test_gator_core.py::TestIsEnterpriseActive pins the semantics.

    Args:
        gator_dir: Path to the repo's .gator/ directory.

    Returns:
        True when the marker exists AND parses AND has `enabled: true`.
        False on any other outcome.
    """
    marker = Path(gator_dir) / "enterprise.json"
    ...
    # JSON root must be an object — `payload.get()` on a list/number/string
    # would raise AttributeError. Codex Phase 4b review flagged that a
    # valid-but-wrong-shape marker (e.g. `[]`, `42`, `"foo"`) crashed the
    # helper instead of returning False. Fail-closed extends to shape.
...
def git(*args, cwd=None):
    ...
    Uses explicit UTF-8 decoding with error replacement to survive git
    output containing non-cp1252 bytes on Windows (where the default
    locale codec is cp1252). Same pattern as `dashboard/helpers.py::git_run`
    which fixed this same class of bug for dashboard code paths.
    """
    try:
        result = subprocess.run(
            ["git"] + list(args),
