...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # GET /api/repo/<name>/blueprint?level=<n>[&focus=<id>] — Blueprints 2.0
        # Release A (v2.11.0): level=1 only, returns the shipped Gator L1 charter
        # map for the Gator source repo, structured empty-state for others.
        # Non-1 level returns 501 (Release B/D/E ship higher levels).
        # No parser subprocess in Release A — pure static payload.
        if path.startswith("/api/repo/") and path.endswith("/blueprint"):
            repo_name = path[len("/api/repo/"):-len("/blueprint")]
            if not repo_name:
                self._send_json({"error": "repo name required"}, 400)
                return
            repo_path = _resolve_repo_path(repo_name, _REGISTRY_REPOS)
            ...
            if level != 1:
                self._send_json({
                    "error": f"level {level} not yet implemented",
                    "reason": "release-b-plus",
                }, 501)
                return
            # Gator-source-repo detection: is the shipped blueprint dataset
            # discoverable on-disk under this repo's src/ tree? If yes, this
            # IS the Gator source repo (per plan D3 recommendation — on-disk
            # artifact discovery, not name-string matching).
            repo_data_file = Path(repo_path) / "src" / "gator_command" / "scripts" / "dashboard" / "blueprint" / "l1-data.json"
            repo_positions_file = Path(repo_path) / "src" / "gator_command" / "scripts" / "dashboard" / "blueprint" / "l1-positions.json"
            if not (repo_data_file.is_file() and repo_positions_file.is_file()):
                self._send_json({
                    "level": 1,
                    "status": "unavailable",
                    "reason": "release-b-pending",
                    "message": (
                        "Blueprints for this repo aren't available yet. "
                        "The Release B charter parser will generate one from "
                        "your .gator/charters/*.md; ships in a subsequent release."
                    ),
                })
                return
            # Read + merge the shipped Gator dataset. Read from the RUNNING
            # dashboard's installed copy (DASHBOARD_DIR), not from the repo —
            # they should be byte-identical in-tree, but the installed copy
            # is what the running CLI just proved it can parse.
            try:
                data_file = DASHBOARD_DIR / "blueprint" / "l1-data.json"
                positions_file = DASHBOARD_DIR / "blueprint" / "l1-positions.json"
                ...
                return
            # Merge positions into node objects by id (positions are an overlay
            # so the Release B parser can regenerate data without stomping them).
            pos_map = positions.get("positions", {})
            for node in data.get("nodes", []):
                node_pos = pos_map.get(node["id"])
                if node_pos:
                    node["x"] = node_pos["x"]
                    node["y"] = node_pos["y"]
            data["canvas"] = positions.get("canvas", {"width": 1180, "height": 880})
