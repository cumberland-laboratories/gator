...
- **Plan A — Dashboard UI test harness** (`8f2b9c9`). Playwright + subprocess-per-child harness with isolated fleet: `_harness.build_dashboard_fleet` builder, `read_ready_url` parsing the `Ready on http://127.0.0.1:<port>/` protocol line at column 0, `--port 0` kernel-assigned-port path in `gator-dashboard.py::main`, `/api/__gator_debug/registry_state` gated on `GATOR_DASHBOARD_DEBUG=1`, direct-script `subprocess.Popen` (no CLI wrapper) so kill signals hit the actual server, session/function-scoped fleet fixtures with 2 seeded repos + 15 inaccessible scratch entries. Registry-isolation TRIPWIRE guards the operator's real `~/.gator/dashboard-repos.json`.
- **Plan B1 — Safe content-transport for `/files`, `/file`, `/raw`, `/history`** (six commits `dcf3a16..6efe9b9`). Single URL parser (`_parse_request`) at the dispatch boundary; namespace-safe `parse_logical_path` with three exact-case namespace roots + case-alias reject + ADS colon + Win32 trailing-dot/space + DOS device (via `_is_reserved_windows_component` two-layer check) + drive-relative + backslash/control-char/rooted-path rejects; `is_browsable` allowlist with governance-root guard; reparse-point-aware `_iter_scanner_files` closing the Windows-junction bypass class; historical `_reparse_ls_tree_entry` for `git ls-tree` symmetry; `_contained_repo_path` with per-component reparse-point walk closing in-repo alias bypass; immutable-object-id `resolve_version_ref`; byte-preserving `git_show_at_ref`; transport headers (`Content-Type` exact + `X-Content-Type-Options: nosniff` unconditional + `Cache-Control: no-store` on every `?version=`-consuming response); four B1-owned handler methods (`_handle_files`/`_handle_file`/`_handle_raw`/`_handle_history`) migrated into `do_GET` in place. Three Codex remediation rounds hardened the F1 in-repo reparse-alias fix, `no-store` on `/history?version=` rejections, `FileNotFoundError`/`PermissionError` → 404 taxonomy with cross-platform in-process branch pins, and a Python 3.9 compat fix for `_is_reparse_point`. Slice 1 unit pins (70) + Slice 2 HTTP integration (46) + Slice 3 platform-specific (13 executed / 10 POSIX-skipped locally) + Plan A harness (22 + 1 skip) = 148 pass + 11 skip on Windows at the B1 checkpoint.
- **Plan B2 — Sandboxed HTML preview** (four commits `4c8b900..c51a1d9`). Server-side `apply_html_csp_headers` seam on `text/html*` responses in `_handle_raw`: `_B2_CSP_EMBEDDED` (no `sandbox` directive; iframe attribute owns isolation) selected on `Sec-Fetch-Dest: iframe`; `_B2_CSP_EXTERNAL` with `sandbox allow-scripts;` prepended for top-level navigation; `Vary: Sec-Fetch-Dest` unconditional. `apply_response_headers` (B1) byte-exact preserved; fast-matrix `_DYNAMIC_CODE_PATTERNS` audit walks both shipped-template roots and blocks `Function`-constructor / `eval` / string-arg timers before a browser would. Browser-side, `views/repo.js::loadFile` HTML branch renders inline via `<iframe sandbox="allow-scripts">` with three controls (Copy path, Refresh cache-buster preserving `?version=`, Open in new tab); module-scope helpers `buildRawUrl` / `copyPathFor` / `bindHtmlPreviewControls`. Three Codex remediation rounds folded into `c51a1d9` drove: postMessage-relay CSP-violation detector (child pushes local + `window.parent.postMessage`; parent's `message` listener aggregates; compat matrix reads parent aggregate to dodge `frame.evaluate`'s no-default-timeout hang on shipped templates); production-path `_click_file_via_repo_view` helper (retargets 4 existing pins at the shipped click path + adds 5 r14 §M1 pins); charter Wasm-audit claim narrowed to match `_DYNAMIC_CODE_PATTERNS`; copy-btn `setTimeout` `e.currentTarget` null-deref fixed via synchronous button capture; R2 two-lane liveness (child-side + parent aggregate) + browser-captured popup CSP + rendered-pixel iframe sizing (`getBoundingClientRect().height >= 400`) alongside computed-style diagnostic; R3 async-flow copy-path pin. 18 Playwright pins in `test_html_preview.py`.
- **Plan C — Responsive shell + sidebar** (`b46d1ef` + 8 R1-R8 charter remediation rounds + `f4f788f` Slice 2 + `5c9f7c2` Slice 3 + 8 R1-R8 remediation rounds). Route-scoped scroll ownership: `showView(name)` in `dashboard.js` toggles `.route-repo` on `#view-slot` for `name === "repo" || name === "docs"`; the CSS flips `#view-slot.route-repo` to `overflow: hidden; display: flex; padding: 0` so `.repo-browser` can flex-fill the slot. Flex chain — `.repo-browser` / `.repo-content` / (`.repo-markdown` | `.repo-search-results` | `.repo-iframe`) all `flex: 1 1 auto; min-height: 0` at every level except the leaf, with exactly ONE leaf owning vertical scroll per view. Viewport-math sizing on `.repo-browser` (the v2.11.1 80%-zoom-required regression's root cause) removed. Mobile media query at end-of-file (load-bearing placement — must follow every `.repo-*` base declaration to win source-order); `#view-slot.route-repo` on mobile uses `overflow: visible` shorthand (both axes) to avoid CSS overflow-axis normalization promoting mixed `hidden`/`visible` to `auto`. Iframe 400px floor from B2 retained on every viewport (Collision B → B1 belt-and-suspenders alongside Plan C's flex chain). Collapsible sidebar with resize + persistence: canonical `renderSidebarShell(sidebarEl, innerHtml)` wrapper installs the collapse button as the sidebar's first child; five pre-existing `sidebar.innerHTML = ...` sites migrated through the wrapper; `toggleSidebarCollapse` flips `_sidebarCollapsed` + persists to `localStorage["gator-sidebar-collapsed:<repoName>"]`; `restoreSidebarState(repoName)` establishes defaults BEFORE the localStorage try block, then reads both stored values into local candidates and commits atomically only after all reads succeed (R2 + R3 hardening for storage-throw contexts); `initResizeHandle` writes to `document.documentElement`'s `--repo-sidebar-width` CSS variable (single source of truth — beats `.repo-sidebar.collapsed { width: 32px }` by normal specificity without needing `!important`) and persists to localStorage on mouseup. `.repo-sidebar.collapsed { min-height: 32px }` ensures the absolute-positioned collapse button (24×24 at top:4px) stays reachable on mobile block layout (R1 F1 fix). Adjacent-sibling combinator `.repo-sidebar.collapsed + #repo-resize-handle { display: none }` hides the sibling resize handle on collapse. 23 Playwright pins in `test_responsive_shell.py`.

### Fixed

- **v2.11.1 80%-zoom-required regression** — nested scrollbars appeared whenever chrome height (topbar + banners + snapshot notice) changed because `.repo-browser` was pinned to `height: calc(100vh - 80px)` instead of flex-sized. Plan C Slice 1 replaces the viewport-math sizing with the flex chain.
- **Windows-junction bypass class** — pre-B1 `rglob` walker followed reparse points; `_iter_scanner_files` now rejects junctions/mount-points before descent and `_contained_repo_path` walks unresolved components rejecting reparse points before resolve, closing an in-repo policy alias where a symlink like `source/public → .gator/sessions/_active` would authorize on the LOGICAL path but resolve to protected content.
- **B2 CSP-violation detector reading wrong document** — R1 F1: the pre-remediation detector read `window.__csp_violations` on the parent document, but CSP violations inside a sandboxed child frame append to the child's array. The compat matrix silently reported every template as clean regardless of what the child produced. Post-fix uses postMessage relay.
- **B2 production-path pin gap** — R1 F2: the initial B2 pins asserted against test-built iframes with hardcoded sandbox attributes, so a regression in the shipped `loadFile()` HTML branch would have gone undetected. `_click_file_via_repo_view` retargets the pins at the production trust boundary.
- **B2 copy-btn `setTimeout` null-deref** — R1 F4: `Event.currentTarget` is cleared post-listener-dispatch; the 1.5s icon-restoration timer fired against `null` and threw. Synchronous button capture at handler entry fixes it.
- **Plan C R1 F1 — mobile collapse strands expand button** — `.repo-browser { display: block }` on mobile + `.repo-sidebar-inner { display: none }` on collapse = sidebar height 0; `overflow: hidden` clipped the button box. `min-height: 32px` on `.repo-sidebar.collapsed` guarantees the 28px-total-footprint button box stays inside the clipped region. Pinned by `test_mobile_collapse_expand_button_remains_clickable`.
- **Plan C R1 F2 — SPA-nav sidebar width leaks between repos** — `restoreSidebarState` never removed the prior repo's `--repo-sidebar-width` value on the document root. Same-page navigation from alpha (with saved 400px) to beta (no saved width) rendered beta at 400px. Fixed with unconditional `removeProperty` before consulting new-repo storage. Pinned by `test_sidebar_width_does_not_leak_between_repos_on_spa_nav`.
- **Plan C R2 + R3 storage-throw HIGH** — R2: defaults must be established BEFORE the localStorage try block (a throw on the first `getItem` would leave prior state). R3: atomic overlay — read both stored values into locals + validate FIRST, then commit both only after all reads succeed (a throw on the second read would otherwise leave a partial overlay). Pinned by `test_sidebar_state_falls_back_to_defaults_when_storage_throws` and `test_sidebar_state_atomic_when_second_read_throws`.
- **Plan C R1 F1 — search-results container clipped** — `views/repo.js::runCrossDocSearch` rendered results in an unclassed `<div>`; under Slice 1's `.repo-content { overflow: hidden }` parent rule, long results were clipped with no scroll owner. Fixed by attaching `class="repo-search-results"` to the outer container so the scroll-owner CSS rule (`flex: 1 1 auto; min-height: 0; overflow-y: auto; overflow-x: auto`) applies. Pinned by `test_search_results_container_has_scroll_owner_class`.
- **B1 `_is_reparse_point` Python 3.9 crash** — `Path.stat(follow_symlinks=)` kwarg is 3.10+; `pyproject.toml` declares `>=3.9`. Fix routes `Path`/`str` inputs through `os.stat(os.fspath(x), follow_symlinks=False)` while keeping the `DirEntry` fast path.

### Changed

- `.repo-content` — was `flex: 1; min-width: 0; overflow-y: auto; overflow-x: auto`; now `flex: 1 1 auto; min-width: 0; min-height: 0; overflow: hidden; display: flex; flex-direction: column`. Scroll delegated to leaf children (`.repo-markdown` / `.repo-search-results` / `.repo-iframe`).
- `.repo-sidebar` base rule — `width: 220px` → `width: var(--repo-sidebar-width, 220px)`; `position: relative` added for absolute-positioned collapse button anchoring.
- `views/repo.js` HTML file rendering — v2.4.5's `window.open(rawUrl, "_blank", "noopener")` default (immediate new-tab) replaced with B2's inline sandboxed iframe as the default; Open-in-new-tab retained as an explicit escape hatch (`noopener,noreferrer`).
- `dashboard.js::showView(name, extra)` — added `.route-repo` class management on `#view-slot` for Repo and Docs routes.

### Compatibility

- **Test suite** — `tests/test_dashboard_ui/`: **189 passed, 11 skipped** on Windows / Python 3.13; +43 net vs post-B1 baseline of 148. No regressions in the broader dashboard test surface.
- **User-facing** — Repo/Docs routes now scroll inside the content pane instead of the whole view-slot; sidebar gains a collapse button that persists per repo; resize width now persists across sessions. HTML files render inline in a sandboxed iframe rather than opening in a new tab. Users on the fleet see the responsive layout + sidebar controls automatically on next Dashboard restart.
- **Storage-restricted contexts** — private browsing, sandboxed frames, cookies-disabled contexts: sidebar renders at the base defaults (expanded, 220px) via the atomic-overlay + pre-try-defaults contract in `restoreSidebarState`. Previously a `SecurityError` from `localStorage.getItem` could leak prior-repo state across SPA navigation.
- **No breaking API changes.** No CLI subcommand changes. Enterprise-cli unchanged.

### Notes

- Every plan shipped through Codex enforcer remediation rounds. The final Slice 3 R1-R8 train closed with the R8 enforcer verdict of "clean. No findings." Cumulative remediation moved from real HIGH-severity implementation defects (mobile collapse strand; SPA-nav width leak; storage-throw fallback ordering + atomic overlay) through charter-accuracy tightening (probe vs inspection attribution, coverage-branch enumeration, stable-identifier sweeps).
- 23 Playwright pins in `test_responsive_shell.py` cover the Slice 1/2/3 invariants; deferred pins that would extend coverage to full Plan C §7 parity are enumerated in the pin file's docstring and in the "Responsive shell + sidebar" charter section (Fleet-scroll-at-900×400, initial-load-error preserves sidebar state, polling-refresh preserves collapse button, 250px minimum-usable-size, invalid-persisted-width fallback, mobile bounded-shell scroll-owner). These are legitimate follow-on coverage for a future slice; each has a named identifier a follow-on plan can target.
- Charter sweep for prohibited line-locator forms (per constitution's Charter Rules on stable identifiers) landed as part of the Slice 3 R6-R8 remediation rounds — `scripts-dashboard.md` now contains zero `filename.ext:N`, `line N`, `line ~N`, or `lines N-M` references. Similar debt likely remains in other charters and warrants a separate directed sweep.
...
- **Protocol contract** `contracts/schemas/gator-blueprint-html-v1.md`. Governs every HTML artifact under `.gator/blueprints/`: four doc classes (`charter-map`, `feature-blueprint`, `procedure-visual`, `reference-explainer`), four status values (`current`, `historical`, `exploratory`, `generated`), 8 required `<meta>` tags (`gator-schema`, `gator-doc-class`, `gator-status`, `gator-updated-at`, `gator-title`, `gator-repo`, `gator-generated-by`, `gator-question`), and layout / styling / interaction / storage invariants that both templates and hand-authored artifacts must satisfy.
- **Two HTML templates** under `templates/gator-starter/blueprints/`:
  - `_template.html` — interactive blueprint (map + sidebar + narrative), ~400 lines, self-contained (no external CSS/JS), click-to-isolate 1-hop-neighborhood interaction, agent-followable content-fill via `NODES` and `EDGES` arrays in the inline `<script>`.
  - `_template-narrative.html` — non-blueprint narrative (header + sequential sections + optional callouts + optional static diagram slot), ~220 lines, self-contained, includes numbered `.steps` list for `procedure-visual` doc-class use.
- **D11 two-lane triage**: node-and-edge structure that benefits from click-to-isolate → interactive template; everything else → narrative template. Same protocol, same discovery, different visual shape.
- **`.gator/blueprints/charter-map.html`** — Gator's own L1 charter map authored per the new protocol (13 nodes / 29 edges, one-time conversion from the retired v2.11.0 `l1-data.json` + `l1-positions.json`). Source-repo only; fleet repos hand-author their own using the templates.
- **Authoring procedure** `templates/gator-starter/procedures/authoring-html-artifacts.md`. When to use HTML vs markdown; D11 triage; required metadata; approved section patterns per template; storage rules; status labeling; how to fill each template; agent checkpoints; explicit "no new `.md` blueprints" rule.
- **Compat test** `contracts/compatibility/test_blueprint_html_schema.py`. Twelve tests validating `.gator/blueprints/*.html` files against the protocol — schema tag, required `<meta>` tags, legal doc-class, legal status, ISO-8601 `updated-at`, no `==TODO==` placeholders survive to shipped, no external stylesheets / scripts, charter-map discovery, and a scaffolding-exclusion contract (`_template.html` + `_template-narrative.html` filenames land at `.gator/blueprints/` on v2 repos as `USER_VISIBLE_SCAFFOLDING` but carry `==TODO==` placeholders by design — `SCAFFOLDING_FILENAMES` filters them from discovery so fleet-repo compat runs skip cleanly instead of failing on template placeholders). **Walks `.gator/blueprints/` only, permanently** — vault artifacts are un-gated (exploratory by design). Skips gracefully on repos with no non-scaffolding HTML blueprints.
- **New `contracts.md` invariant** — "Shipped-template surfaces reference governed-repo paths only": any file under `templates/gator-starter/` that lands in a fleet repo (README, HTML template bodies, procedures, scaffolding comments) must reference paths that exist in a gatorized repo. Source-tree paths (`contracts/schemas/…`, `src/gator_command/…`, `pytest contracts/compatibility/…`) don't exist in fleet repos and are dead at the point of use. Sole accepted exception is descriptive-not-prescriptive framing ("the canonical contract lives in the Gator source tree at `X` and is not shipped here"). Codifies the discipline reviewed across two whiteboard rounds during this release.

### Changed

- **`USER_VISIBLE_SCAFFOLDING` extended** in `gator_layout.py` (and template copy) from `{"README.md", "_template.md"}` to include both HTML templates as filename-exact scaffolding peers. Without this the templates would land at `.gator/.includes/blueprints/` on v2 repos, invisible where agents look for authoring inputs. Two regression pins in `tests/test_layout.py`. New TRIPWIRE in `scripts-layout.md` names the "silent drift" class and the fixture-pairing rule for future additions.
- **`blueprints/README.md` rewrite** — replaces the markdown-only structure block with the HTML-only direction, D11 triage table, template pointers, storage rules, discovery note. Preserves a "legacy markdown blueprints" section pointing at `_template.md` as historical reference.
- **Shipped documentation retargeted at governed-repo paths across two whiteboard rounds** — the authoring procedure, blueprints/README.md, and both HTML template comments + `<body>Protocol notes</body>` sections originally linked at source-tree-only paths (`contracts/schemas/…`, `pytest contracts/compatibility/…`, `src/gator_command/templates/…`) that don't exist in gatorized fleet repos. Every shipped link now points at governed-repo paths (`.gator/procedures/…`, `.gator/blueprints/README.md`); the pytest invocation is replaced with a plain checkpoint description; template body-text `<section>Protocol notes</section>` now points at the shipped procedure and carries a `==TODO==` telling the author to delete the section if they don't want protocol notes in the published artifact. Source-tree references retained only as descriptive-not-prescriptive context (see the new `contracts.md` invariant).
- **Charter surface adjustments** — `scripts-dashboard.md` retired-view entry replaced with a short "HTML artifact rendering (v2.12.0 onward — no dedicated view)" pointer to the v2.4.5 file browser; `scripts-cross-cutting.md` "Repo-Scoped Dashboard Endpoints" pattern reframed with retired Blueprints as historical precedent (the failure mode the pattern prevents); `scripts-fleet-intelligence.md` updated with a v2.11.0-retirement note; `scripts-repo-lifecycle.md` mixed-directory note extended to list both HTML template filenames; `contracts.md` gains schema + test entries + the scaffolding-exclusion contract note + the new shipped-template-reference invariant.

### Removed

- **v2.11.0 Blueprints surface fully retired** (D1 ratified full retire, no repurpose). Deleted files: `src/gator_command/scripts/dashboard/views/blueprint.js` (~322 lines), `src/gator_command/scripts/dashboard/blueprint/l1-data.json`, `src/gator_command/scripts/dashboard/blueprint/l1-positions.json`, `tests/test_blueprint_view.py` (16 tests), empty `dashboard/blueprint/` directory. `dashboard.html` — removed Blueprints sidebar item + `<script src="views/blueprint.js">`. `dashboard.js` — removed `blueprint` from `VIEW_META`, `blueprintTab` DOM ref, `showView()` branch, un-dim hook. `dashboard.css` — removed ~262 lines of `bp-*` CSS. `dashboard/snapshot.py` — removed `blueprint_js` from read chain + regex sequence. `gator-dashboard.py` — removed the `/api/repo/<name>/blueprint` handler (~80 lines).

### Compatibility

- **Existing markdown blueprints coexist untouched** (D2 ratified). Any repo's existing `*.md` blueprints continue to work; the protocol only governs new HTML content going forward. `_template.md` stays shipped as the "legacy" reference.
- **New-tab rendering** (D4 ratified). HTML blueprints open through the existing v2.4.5 `.html` file surface in a new browser tab — no iframe embedding, no Dashboard view. This keeps the Dashboard a thin browser and inherits the same discovery + browse UX as any other repo file.
- **Machines pipx-upgrading from v2.11.1** lose the Blueprints sidebar item automatically. Any repo that scaffolded v2.11.0's `dashboard/blueprint/l1-data.json` at the flat root would still see it, but the Dashboard no longer references it. `gator update` on v2 repos lands both new HTML templates alongside `_template.md` at `.gator/blueprints/`.
- **Test suite: 975 pass + 3 skip + 2 xfail** (v2.11.1 baseline was 977; net -2 = -16 retired blueprint view tests + 2 new test_layout scaffolding pins + 12 new compat schema tests).

### Notes

- Plan: `.gator/vault/artifacts/2026-09-02-html-artifact-protocol-implementation-plan.md` r5 (Architect ratified D1 retire, D2 coexist, D4 new-tab, D11 HTML-only with two-lane triage). Whiteboard rounds r2 (source-repo-only reference impl + `USER_VISIBLE_SCAFFOLDING` extension), r3 (crisp authoring-vs-generation split), r4 (permanent vault un-gating + migration-matrix cleanup) all applied.
- Two post-Release-A whiteboard fixes landed pre-bump: **round 1 (`af6163d`)** added the scaffolding-exclusion class to the compat test + retargeted the procedure and template comments at governed-repo paths; **round 2 (`9f31db6`)** caught three more references the reviewer flagged as higher-severity — three README links + two template `<body>` sections that render into every newly-authored artifact — and added the codifying `contracts.md` invariant so the discipline survives future template edits.
- Bundle chain: **Release A (this, v2.12.0)** = protocol + two templates + authoring procedure + `USER_VISIBLE_SCAFFOLDING` extension + charter-map.html + v2.11.0 retirement. Deferred: **Release B (v2.13.0)** feature-blueprint generation procedure; **Release C (v2.14.0)** Dashboard HTML artifact index; **Release D (v2.15.0+)** procedure-visual + reference-explainer worked examples.
